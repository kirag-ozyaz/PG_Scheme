...
//                wordDoc.Visible = true;
                // в метод-параметр передаем документ созданный на основе шаблона
                fillWordDoc(wordDoc);
            }
            catch (Exception error)
            {
                if (wordDoc != null) { wordDoc.Close(); }
                MessageBox.Show("Ошибка при вставке другого документа Word из временного файла в текущий документ Word. Подробности " + error.Message + " " + error.StackTrace);
                throw error;
            }
