...
internal class FormChooseDamageCharacter : FormBase
{
    ...
    internal FormChooseDamageCharacter(int? nullable_2 = new int?(), List<int> list_2 = null, int? nullable_3 = new int?())
    {
        this.InitializeComponent();
    ...
    private void FormChooseDamageCharacter_FormClosing(object sender, FormClosingEventArgs e)
    {
        if (base.DialogResult == DialogResult.OK)
        {
            ...
            foreach (DataGridViewRow row in (IEnumerable) this.dgv.Rows)
            {
                ...
                {
                    this.list_0.Add(Convert.ToInt32(row.Cells[this.dataGridViewTextBoxColumn_0.Name].Value));
                }
            }
            if (((this.list_1 == null) && !this.nullable_1.HasValue) && (this.list_0.Count == 0))
            {
                ...
                base.SelectSqlData(table, true, string.Format("where idParent = {0} and typeDoc = {1} \r\n                        and not exists(select id from tJ_DamageCharacter where idDAmage = tj_damage.id)", this.nullable_0, 0x752), null, false, 0);
    ...
    private void method_1()
    {
        ...
        {
            str = " and dd.id <> " + this.nullable_1.Value.ToString();
        }
        string str2 = "";
