    ...
    public class FormJournalDamage : FormBase
    {
        ...
        protected override void ApplyConfig(XmlDocument xmlDocument_0)
        {
            ...
            if (node != null)
            {
                ...
                {
                    if (list.Contains(((Struct10) this.checkedListBoxDivision.Items[i]).int_0))
                    {
                ...
                if (node2 != null)
                {
                    ...
                    {
                        if (list2.Contains(((Struct10) this.checkedListBoxDivisionApply.Items[j]).int_0))
                        {
        ...
        private void bindingSource_0_ListChanged(object sender, ListChangedEventArgs e)
        {
            ...
            this.txtLegendApply.Text = this.dsDamage.vJ_Damage.Compute("count(id)", string.Format("{0} isApply = 1", str)).ToString() + " - устранено";
            this.txtLegendInWork.Text = this.dsDamage.vJ_Damage.Compute("count(id)", string.Format("{0} (isApply is null or isApply = 0) \r\n                    and (datePlanEnd is null or datePlanEnd >= '{1}')\r\n                    and not (idReqForRepair is not null and idReason is null)", str, DateTime.Now.Date.ToString("yyyy-MM-dd") + " 00:00:00")).ToString() + " - в работе";
            this.txtLegendOverdue.Text = this.dsDamage.vJ_Damage.Compute("count(id)", string.Format("{0} (isApply is null or isApply = 0) \r\n                    and (datePlanEnd < '{1}')\r\n                    and not (idReqForRepair is not null and idReason is null)", str, DateTime.Now.Date.ToString("yyyy-MM-dd") + " 00:00:00")).ToString() + " - просрочено";
            this.txtLegendUnExec.Text = this.dsDamage.vJ_Damage.Compute("count(id)", string.Format("{0} (isApply is null or isApply = 0) \r\n                    and (idReqForRepair is not null and idReason is null)", str)).ToString() + " - неоформлено";
            this.txtLegendNoSESNO.Text = this.dsDamage.vJ_Damage.Compute("count(id)", string.Format("{0} (isNoSESNO = 1)", str)).ToString() + " - нет сетей Горсвет";
        }
        ...
        protected override XmlDocument CreateXmlConfig()
        {
            ...
            foreach (object obj2 in this.checkedListBoxDivision.CheckedItems)
            {
                ...
                attribute2.Value = struct2.int_0.ToString();
            ...
            foreach (object obj3 in this.checkedListBoxDivisionApply.CheckedItems)
            {
                ...
                attribute3.Value = struct2.int_0.ToString();
        ...
        private void InitializeComponent()
        {
            ...
            this.bindingSource_0.DataSource = this.dsDamage;
            ...
            this.toolBtnSettingMail.Image = global::Properties.Resources.mail;
        ...
        private void method_2(object sender, EventArgs e)
        {
            ...
            if (item.Tag != null)
            {
                this.method_3(Convert.ToInt32(item.Tag), -1, false);
            }
        }
        ...
        private void method_3(int int_1, int int_2 = -1, bool bool_0 = false)
        {
            switch (int_1)
            {
                case 0x579://1401
                {
                    FormDamageLVAddEdit form = new FormDamageLVAddEdit(int_2);//c000260
                ...
                case 0x57a: //1402
                {
                    FormDamageHVAddEdit form2 = new FormDamageHVAddEdit(int_2);//c00025b
                ...
                case 0x734:
                {
                    FormDefectAddEdit form3 = new FormDefectAddEdit(int_2, int_1);//c00025f
                        form3.SqlSettings=(this.SqlSettings);
                    form3.MdiParent = base.MdiParent;
                    form3.method_2(bool_0);
                    if (!bool_0)
                    {
                ...
                case 0x752:
                {
                    FormActDetectionAddEdit form1 = new FormActDetectionAddEdit(new int?(int_2)); //c0002d6
                    form1.SqlSettings=(this.SqlSettings);
                    form1.MdiParent = base.MdiParent;
                    form1.method_1(bool_0);
                    form1.Show();
                    return;
                }
            }
        ...
        private void Damage_FormClosed(object sender, FormClosedEventArgs e)
        {
            if ((sender is Form) && (((Form) sender).DialogResult == DialogResult.OK))
            {
                ...
                {
                    num = ((FormDamageLVAddEdit) sender).method_0();
                }
                if (sender is FormDefectAddEdit)
                {
                    num = ((FormDefectAddEdit) sender).method_0();
                }
                this.method_6(num);
            }
        }
        ...
        {
            Struct10 struct2;
            if ((int_1 == -1) && (this.dgvDamage.CurrentRow != null))
            {
                int_1 = Convert.ToInt32(this.dgvDamage.CurrentRow.Cells[this.idDataGridViewTextBoxColumn.Name].Value);
            }
            string str = "";
            ...
            if ((this.checkedListBoxDivision.CheckedItems.Count > 0) && (this.checkedListBoxDivision.CheckedItems.Count < this.checkedListBoxDivision.Items.Count))
            {
                ...
                foreach (object obj2 in this.checkedListBoxDivision.CheckedItems)
                {
                    ...
                    {
                        struct2 = (Struct10) obj2;
                        str2 = struct2.int_0.ToString();
                    }
                    else
                    {
                        struct2 = (Struct10) obj2;
                        str2 = str2 + "," + struct2.int_0.ToString();
                    }
                }
                str = str + string.Format(" and (idDivision in ({0}))", str2);
            }
            ...
            if (listChecked.Count > 0)
            {
                ...
                {
                    str = str + string.Format(" and (idSchmObj in ({0}) or idSub in ({0}))", str3);
                }
                else
                {
                    str = str + string.Format(" and (idSchmObj in ({0}))", str3);
                }
            }
            if (this.checkedListBoxDivisionApply.Items.Count > 0)
            {
                ...
                if (this.checkedListBoxDivisionApply.CheckedItems.Count < this.checkedListBoxDivisionApply.Items.Count)
                {
                    ...
                    foreach (object obj3 in this.checkedListBoxDivisionApply.CheckedItems)
                    {
                        ...
                        {
                            struct2 = (Struct10) obj3;
                            str4 = struct2.int_0.ToString();
                        }
                        else
                        {
                            struct2 = (Struct10) obj3;
                            str4 = str4 + "," + struct2.int_0.ToString();
                        }
                    }
                    ...
                    {
                        str = str + string.Format(" and (idDivisionApply in ({0}) or (isLaboratory = 1 and typedoc in (1403,1844))\r\n                                                       or (idDivisionApply is null and (isLaboratory is null or isLaboratory = 0)))", str4);
                    }
                    else
                    {
                        str = str + string.Format(" and (idDivisionApply in ({0}) or\r\n                                                          (idDivisionApply is null and (isLaboratory is null or isLaboratory = 0)))", str4);
                    }
                }
            }
            base.SelectSqlData(this.dsDamage.vJ_Damage, true, str + " order by dateDoc desc", null, false, 0);
            this.dgvDamage.SearchGrid(this.idDataGridViewTextBoxColumn.Name, int_1.ToString(), true);
        }
        ...
        private void toolItemDel_Click(object sender, EventArgs e)
        {
            if (this.bindingSource_0.Current != null)
            {
                ...
                {
                    int num = Convert.ToInt32(this.dgvDamage.CurrentRow.Cells[this.idDataGridViewTextBoxColumn.Name].Value);
                    if (base.DeleteSqlDataById(this.dsDamage.tJ_Damage, num))
                    {
                        this.method_5(num);
                        this.method_6(-1);
        ...
        private void toolItemActDetectionAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                DataRow row = ((DataRowView) this.bindingSource_0.Current).Row;
                if (Convert.ToInt32(row["TypeDoc"]) == 0x57a)
                {
                    int num = Convert.ToInt32(row["id"]);
                    FormChooseDamageCharacter form = new FormChooseDamageCharacter(new int?(num), null, null);
                    form.SqlSettings = (this.SqlSettings);
                    if (form.ShowDialog() == DialogResult.OK)
                    {
                        FormActDetectionAddEdit form1 = new FormActDetectionAddEdit(new int?(num), form.method_0().ToList<int>());
                        form1.SqlSettings = (this.SqlSettings);
                        form1.MdiParent = base.MdiParent;
                        form1.Show();
                    }
