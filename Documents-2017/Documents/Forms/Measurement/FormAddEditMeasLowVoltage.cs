...
//        private int IdBus;
        ...
        private void bsBusesTransf_CurrentChanged(object sender, EventArgs e)
        {
            ...
            {
                EnumerableRowCollection<DataRow> source = base.SelectSqlData("tSchm_ObjList", true, string.Format("WHERE idParent = {0} AND Deleted = ((0)) AND (id = {1} OR Tag = {1})", this.IdSubstation, this.IdBus)).AsEnumerable();
                    ...
                    if (!string.IsNullOrEmpty(str2))
                    {
                        ...
                        if (this.dsMeasLowVoltage.vJ_TransfByCell.Count > 0)
                        {
                            ...
                            this.dsMeasLowVoltage.vJ_CellByBus.RemovevJ_CellByBusRow((from r in this.dsMeasLowVoltage.vJ_CellByBus
                                                                               ...
                                                                               select r).First<DataSetMeasurementN.vJ_CellByBusRow>());
                        }
        ...
        {
            if ((e.RowIndex >= 0) && ((this.dgvAbnObj[this.dataGridViewTextBoxColumn_19.Name, e.RowIndex].Value == DBNull.Value) || (Convert.ToInt32(this.dgvAbnObj[this.dataGridViewTextBoxColumn_19.Name, e.RowIndex].Value) != 0x4a7)))
            {
        ...
        //private StateFormCreate stateFormCreate;
        //[CompilerGenerated]
        //internal StateFormCreate method_12()// get_StateCreate()
        //{
        //    return this.stateFormCreate;
        //}

        //[CompilerGenerated]
        //internal void method_13(StateFormCreate stateFormCreate_1) //set_StateCreate(StateFormCreate stateFormCreate_1)
        //{
        //    this.stateFormCreate = stateFormCreate_1;
        //}
        ...
        private void FillBusesTransf()
        {
            ...
            if (this.StateCreate == StateFormCreate.Edit) //1
            {
                ...
                //foreach (Class6<int, string, int, string, string> current in this.dsMeasLowVoltage.vJ_BusesTransfSchema.Select<DataSetGES.Class121, Class6<int, string, int, string, string>>((C__1.<> 9__41_0 ?? (C__1.<> 9__41_0 = new Func<DataSetGES.Class121, Class6<int, string, int, string, string>>(C__1.<> 9.method_0)))).Except<Class6<int, string, int, string, string>>(this.dsMeasLowVoltage.vJ_BusesTransfs.Select<DataSetGES.Class122, Class6<int, string, int, string, string>>((C__1.<> 9__41_1 ?? (C__1.<> 9__41_1 = new Func<DataSetGES.Class122, Class6<int, string, int, string, string>>(C__1.<> 9.method_1))))).Union<Class6<int, string, int, string, string>>(this.dsMeasLowVoltage.vJ_BusesTransfs.Select<DataSetGES.Class122, Class6<int, string, int, string, string>>(C__1.<> 9__41_2 ?? (C__1.<> 9__41_2 = new Func<DataSetGES.Class122, Class6<int, string, int, string, string>>(C__1.<> 9.method_2)))))
            ...
            if (this.StateCreate == StateFormCreate.Add)
            {
                ...
                this.dsMeasLowVoltage.dtBusesTransf.Rows.Clear();
                //foreach (DataRow row in this.dsMeasLowVoltage.dtBusesTransf.OrderBy<DataSetGES.Class120, string>((C__1.<> 9__41_3 ?? (C__1.<> 9__41_3 = new Func<DataSetGES.Class120, string>(C__1.<> 9.method_3)))).CopyToDataTable<DataSetGES.Class120>().Rows)
       ...
       // [CompilerGenerated]
                    ...
                    foreach (DataRow dataRow in dataTable.Rows)
                    {
                        ...
                        //////while (enumerator.MoveNext())
                        {
                            ...
                            if (dataRow["idCell"] != DBNull.Value)
                            {
                                ...
                                EnumerableRowCollection<string> rows1 = from r in this.dsMeasLowVoltage.vJ_CellByBus
                                                                         where r.id == (int)dataRow["idCell"]
                                                                         select r.Name;
                                if (rows1.Count<string>() > 0)
                                {
                                    tJ_MeasCableRow["NameCell"] = rows1.First<string>();
                                }
                            }
                            ...
                            if (dataRow["idCable"] != DBNull.Value)
                            {
                                ...
                                EnumerableRowCollection<string> rows2 = from r in this.dsMeasLowVoltage.vJ_CableByCell
                                                                          where r.id == (int)dataRow["idCable"]
                                                                          select r.Name;
                                if (rows2.Count<string>() > 0)
                                {
                                    tJ_MeasCableRow["NameCable"] = rows2.First<string>();
                                }
                            }
        ...
        private int CreateNewCableRows(DataSetMeasurementN.tJ_MeasCableDataTable dt, int cell)
        {
            ...
            foreach (var current in orderedEnumerable)
            {
                ...
                row.id = (dt.Rows.Count > 0) ? dt.Min(r => r.id) : -1;
                row.idMeasurement = this.IdMeasurement;
                row.idBus = this.IdBus;
        ...
        private bool SaveData()
        {
            if (this.StateCreate == StateFormCreate.Add)
            {
                ...
                //    }
                //}
                foreach (DataSetMeasurementN.tJ_MeasurementRow current in this.dsMeasLowVoltage.tJ_Measurement)
                {
                    current.EndEdit();
                }
                ...
                foreach (string str in this.dictionaryKeys)
                {
                    ...
                    {
                        foreach (DataSetMeasurementN.tJ_MeasVoltageTransfRow local1 in this.TransfVoltageTables[str])
                        {
                        ...
                        foreach (DataSetMeasurementN.tJ_MeasCableRow local2 in this.MeasCableTables[str])
                        {
                        ...
                        base.InsertSqlData(this.MeasCableTables[str]);
            ...
            else
            {
                ...
                //using (enumerator5 = this.dsMeasLowVoltage.tJ_Measurement.Rows.GetEnumerator())
                //{
                ...
                foreach (string str2 in this.dictionaryKeys)
                {
                    ...
                    {
                        //using (enumerator5 = this.TransfVoltageTables[str2].Rows.GetEnumerator())
                        //{
                        //    while (enumerator5.MoveNext())
                        //    {
                        //        ((DataRow)enumerator5.Current).EndEdit();
                        //    }
                        //}
                        foreach (DataRow dataRow2 in this.TransfVoltageTables[str2].Rows)
                        {
                            dataRow2.EndEdit();
                        }
                        base.UpdateSqlData(this.TransfVoltageTables[str2]);
                        ...
                        //    }
                        //}
                        foreach (DataRow dataRow3 in this.MeasCableTables[str2].Rows)
                        {
                            dataRow3.EndEdit();
                        }
