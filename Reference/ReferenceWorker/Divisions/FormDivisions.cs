...
namespace ReferenceWorker
{
	...
	{
        string TableName = "k_tr_Division";
		...
		private void method_1(TreeNode treeNode_0)
		{
			...
			foreach (DataRow dataRow in this.dataSet.Tables[TableName].Select("ParentId is NULL" + str, "Description"))
			{
		...
		{
			string str = this.toolStripButtonHideDelete.Checked ? "" : " and Deleted = false";
			foreach (DataRow dataRow in this.dataSet.Tables[TableName].Select("ParentId = " + treeNode_0.Tag.ToString() + str, "Description ASC"))
			{
		...
		private string method_3(DataRow dataRow_0)
		{
			...
			string str = this.toolStripButtonHideDelete.Checked ? "" : " and Deleted = false";
			if (this.dataSet.Tables[TableName].Select("ParentId = " + dataRow_0["id"].ToString() + str).Length != 0)
			{
		...
		private void toolStripButtonClose_Click(object sender, EventArgs e)
		{
			...
			this.bs_Division.Filter = filter;
		}
		...
		{
			int num;
			return treeNode_0.Tag != null && int.TryParse(this.treeView.SelectedNode.Tag.ToString(), out num) && (bool)this.dataSet.Tables[TableName].Select("id = " + num.ToString())[0]["deleted"];
		}
		...
		private void toolMenuEdit_Click(object sender, EventArgs e)
		{
			...
			this.formDivisionsAddEdit_0.SqlSettings = this.SqlSettings;
			this.formDivisionsAddEdit_0.method_1(StateFormCreate.Edit);
			this.formDivisionsAddEdit_0.MdiParent = base.MdiParent;
			this.formDivisionsAddEdit_0.Show();
			this.formDivisionsAddEdit_0.FormClosed -= this.ohiuLkgoyw;
			this.formDivisionsAddEdit_0.FormClosed += this.ohiuLkgoyw;
		...
		private void toolMenuDelRest_Click(object sender, EventArgs e)
		{
			...
			{
				DataRow[] array = this.dataSet.Tables[TableName].Select(string.Format("id = {0}", num));
				array[0]["deleted"] = false;
				if (this.treeView.SelectedNode.Parent != null && this.treeView.SelectedNode.Parent.Tag != null)
				{
					DataRow[] array2 = this.dataSet.Tables[TableName].Select(string.Format("id = {0}", (int)array[0]["ParentId"]));
			...
			{
				DataRow[] array2 = this.dataSet.Tables[TableName].Select(string.Format("id = {0} or parentId = {0}", num));
				for (int i = 0; i < array2.Length; i++)
				{
		...
		private void method_9(TreeNode treeNode_0, bool bool_0)
		{
			...
			{
				TreeNode treeNode = (TreeNode)obj;
				this.dataSet.Tables[TableName].Select("id = " + treeNode.Tag.ToString())[0]["deleted"] = bool_0;
		...
		private void toolMenuDelFull_Click(object sender, EventArgs e)
		{
			...
			if (!base.DeleteSqlData(this.dataSet.Tables[TableName]))
			{
				MessageBox.Show("Не удалось внести изменения", "", MessageBoxButtons.OK, MessageBoxIcon.Hand);
			}
			base.SelectSqlData(this.dataSet, this.dataSet.Tables[TableName], true);
		...
		private void method_10(TreeNode treeNode_0)
		{
			...
			{
				TreeNode treeNode = (TreeNode)obj;
				this.dataSet.Tables[TableName].Select("id = " + treeNode.Tag.ToString())[0].Delete();
