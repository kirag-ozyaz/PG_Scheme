...
namespace ReferenceWorker
{
	public partial class FormJobTitleAddEditPadez : FormBase
	{
		...
		private void FormJobTitleAddEditPadez_Load(object sender, EventArgs e)
		{
			if (this.int_0 != -1)
			{
				base.SelectSqlData(this.dsWorker.tR_JobTitle, true, "where id = " + this.int_0.ToString(), null, false, 0);
				if (this.dsWorker.tR_JobTitle.Rows.Count <= 0)
				{
					this.buttonOK.Enabled = false;
					return;
				}
				DataRow dataRow = this.dsWorker.tR_JobTitle.Rows[0];
		...
		{
			if (base.DialogResult == DialogResult.OK && this.dsWorker.tR_JobTitle.Rows.Count > 0)
			{
				XmlDocument xmlDocument = new XmlDocument();
				...
				this.dsWorker.tR_JobTitle.Rows[0]["Padez"] = xmlDocument.InnerXml;
				this.dsWorker.tR_JobTitle.Rows[0].EndEdit();
				base.UpdateSqlData(this.dsWorker.tR_JobTitle);
			}
		}
