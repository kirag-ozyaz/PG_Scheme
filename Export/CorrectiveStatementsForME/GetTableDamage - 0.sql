...
(select d.id idDoc, col.idAbnObj.value('@idAbnObj', 'int') idAbnObj, d.TypeDoc, d.NumRequest,  d.DateDoc
     ...
     , case 
		...
		where d.deleted = 0 and col.idAbnObj.value('@CountPoint', 'varchar(max)') > 0 and d.isSendSiteMe = @isSendSiteMe
...
where doc.deleted = 0  and Year(doc.DateDoc) >= 2019 and doc.isSendSiteMe  = @isSendSiteMe
