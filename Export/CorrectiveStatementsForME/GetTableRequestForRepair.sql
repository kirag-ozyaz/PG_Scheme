...
Запрос получения плановых работ в 2019
вход @Year и @Month
...
(select d.id idDoc, col.idAbnObj.value('@idAbnObj', 'int') idAbnObj 
     ...
     , case 
		...
		left join tJ_RequestForRepairDaily daily on daily.[idRequest] = d.id
		where d.deleted = 0  and d.sendSite = 1 and col.idAbnObj.value('@countPoint', 'int') > 0
		and col.idAbnObj.value('@idAbn', 'varchar(max)') <> ''
		and (d.Crash = 0  or d.crash is null)  
		-- and (@isSendSiteMe is null or d.isSendSiteMe  = @isSendSiteMe)
		and (Year(daily.[dateBeg]) >= 2019 or Year(daily.[dateEnd]) >= 2019) 
		and d.iddivision = 920
...
select doc.id id, doc.numRequestME num, Cast(netRegion.Value as int) name
		...
		, Cast((select distinct  sum(d2.AveragePowerConsumedAbn)
						from AbnObj d2
						where doc.id = d2.idDoc) as numeric(10,2)) as power_total
