...
namespace Passport.Forms.PassportEE
{
	public static class GetData
	{
		...
		public static string GetSSNameFromDB(string string_0)
		{
			SqlConnection sqlConnection = new SqlConnection();
			sqlConnection.ConnectionString = "Data Source=ulges-sql2;Initial Catalog=GES;Integrated Security=True";
			sqlConnection.Open();
			DbDataAdapter dbDataAdapter = new SqlDataAdapter(new SqlCommand("SELECT c.SocrName + '-' + o.name AS name FROM [GES].[dbo].[tSchm_ObjList] AS o INNER JOIN tR_Classifier AS c ON o.TypeCodeId = c.id WHERE c.id in (536, 537, 538, 535, 1034) AND o.name = '" + string_0 + "'", sqlConnection));
			DataTable dataTable = new DataTable();
			dbDataAdapter.Fill(dataTable);
			sqlConnection.Close();
