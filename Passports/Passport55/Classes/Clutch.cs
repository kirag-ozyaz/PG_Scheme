...
namespace Passport.Classes
{
	public static class Clutch
	{
		...
		public static int[] GetAllClutchOnCabSection(SQLSettings settings, int idPassport)
		{
			...
			new SqlDataCommand(settings).SelectSqlData(dsPassport.tP_CabSection, true, string.Format(" WHERE idPassport = {0} AND Deleted = ((0))", idPassport), null, false, 0);
			EnumerableRowCollection<int> enumerableRowCollection = dsPassport.tP_CabSection.Where(new Func<dsPassport.Class58, bool>(Clutch.<>c.<>9.method_0)).Select(new Func<dsPassport.Class58, int>(Clutch.<>c.<>9.method_1));
			EnumerableRowCollection<int> enumerableRowCollection2 = dsPassport.tP_CabSection.Where(new Func<dsPassport.Class58, bool>(Clutch.<>c.<>9.method_2)).Select(new Func<dsPassport.Class58, int>(Clutch.<>c.<>9.method_3));
		...
		public static int Create(SQLSettings settings, int idCable, string Name, TypeClutch type)
		{
			...
			@class.IdParent = idCable;
			@class.TypeCodeId = (int)type;
			@class.idParentAddl = idCable;
			@class.Name = Name;
		...
		public static bool Edit(SQLSettings settings, int idClutch, string name, TypeClutch type)
		{
			SqlDataCommand sqlDataCommand = new SqlDataCommand(settings);
			dsPassport dsPassport = new dsPassport();
			sqlDataCommand.SelectSqlData(dsPassport.tSchm_ObjList, true, "WHERE id = " + idClutch.ToString(), null, false, 0);
			if (dsPassport.tSchm_ObjList.Rows.Count == 0)
			{
				throw new Exception("Не найдена муфта.");
			}
			dsPassport.Class49 @class = (dsPassport.Class49)dsPassport.tSchm_ObjList.Rows[0];
			@class.TypeCodeId = (int)type;
			@class.Name = name;
		...
		public static bool Delete(SQLSettings settings, int idClutch)
		{
			SqlDataCommand sqlDataCommand = new SqlDataCommand(settings);
			dsPassport dsPassport = new dsPassport();
			sqlDataCommand.SelectSqlData(dsPassport.tSchm_ObjList, true, string.Format(" WHERE id = {0}", idClutch), null, false, 0);
			if (dsPassport.tSchm_ObjList.Rows.Count == 0)
			{
				throw new Exception("DeleteClutch: Clutch not found");
			}
			dsPassport.tSchm_ObjList.Rows[0]["Deleted"] = true;
			dsPassport.tSchm_ObjList.Rows[0].EndEdit();
			return sqlDataCommand.UpdateSqlData(dsPassport, dsPassport.tSchm_ObjList);
		}
		...
		public static bool DeletePassport(SQLSettings settings, int idClutchPassport)
		{
			SqlDataCommand sqlDataCommand = new SqlDataCommand(settings);
			dsPassport dsPassport = new dsPassport();
			sqlDataCommand.SelectSqlData(dsPassport.tP_Passport, true, string.Format(" WHERE id = {0}", idClutchPassport), null, false, 0);
			if (dsPassport.tP_Passport.Rows.Count == 0)
			{
				throw new Exception("DeleteClutchPassport: Passport clutch not found");
			}
			dsPassport.tP_Passport.Rows[0]["Deleted"] = true;
			dsPassport.tP_Passport.Rows[0].EndEdit();
