...
namespace Passport.Classes
{
	public static class PassportData
	{
		...
		{
			return (from row in new SqlDataCommand(sqlsettings_0).SelectSqlData("SELECT Description FROM vP_TableColumnDescription WHERE [Table] = '" + string_0 + "' AND [Description] IS NOT NULL").AsEnumerable()
			select row.Field("Description")).ToArray<string>();
		}
	}
}
