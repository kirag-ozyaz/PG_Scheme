...
namespace Passport.Controls
{
	public class PanelPassport : Panel
	{
		...
		private void method_1(int int_1, string string_0)
		{
			try
			{
				...
				if (this.sqlsettings_0 != null)
				{
					...
					{
						this.QaGmzhvawqg = new SqlConnection(string.Format("Data Source={0};Initial Catalog={1};Integrated Security=True", this.sqlsettings_0.ServerDB, this.sqlsettings_0.DBName));
					}
					else
					{
						this.QaGmzhvawqg = new SqlConnection(string.Concat(new string[]
						{
							"Server=",
					...
					this.QaGmzhvawqg.Open();
					DbDataAdapter dbDataAdapter = new SqlDataAdapter(new SqlCommand(string.Format("select * from vPassportPanelShowData where isActive = 1 and Deleted = 0 and idObjList = {0} and Panel = 1", int_1), this.QaGmzhvawqg));
					this.dataTable_0 = new DataTable();
					dbDataAdapter.Fill(this.dataTable_0);
					if (this.dataTable_0.Rows.Count > 0)
					{
						int_1 = (int)this.dataTable_0.Rows[0]["idObjList"];
						...
						for (int k = 0; k < this.dataTable_0.Rows.Count + 1; k++)
						{
							...
							this.int_0 = this.label_0[k, 0].Location.X + this.label_0[k, 0].Width + 20 + this.label_0[k, 1].Width + 12;
							if (this.xKfmzAbUudp < this.int_0)
							{
								this.xKfmzAbUudp = this.int_0;
							}
						...
						{
							this.label_0[l, 1].Location = new Point(this.xKfmzAbUudp - (this.label_0[l, 1].Width + 12), 13 + l * 23);
						}
						base.Width = this.xKfmzAbUudp;
		...
		private SqlConnection QaGmzhvawqg;
