...
namespace Passport.Forms.PassportEE
{
	public class FormPassportEE : FormBase
	{
		...
		private void method_3(Workbook workbook_1)
		{
			...
			this.fhySxofeg2 = GetData.GetFactPower();
			IEnumerable<string> enumerable = from row in transformer.AsEnumerable()
			group row by row.Field("Расположение") into g
			select g.Key;
			int num = 0;
			using (IEnumerator<string> enumerator = enumerable.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					Range range = this.method_1(this.worksheet_0, "H", item);
					if (range != null)
					{
						EnumerableRowCollection<DataRow> enumerableRowCollection = from row in transformer.AsEnumerable()
						where row.Field("Расположение") == item
						select row;
						int num2 = 0;
				...
				if (((Range)range.Cells.get__Default(1, 1)).Column == 11 && range.Row > 19 && range.Row < workbook.Names.Item("end_03", Type.Missing, Type.Missing).RefersToRange.Row)
				{
					if (((Range)range.Cells.get__Default(1, 1)).get_Value(Type.Missing).ToString() == "нет")
					{
						if (worksheet.get_Range("BA" + range.Row.ToString(), Type.Missing).get_Value(Type.Missing) == null || worksheet.get_Range("BA" + range.Row.ToString(), Type.Missing).get_Value(Type.Missing).ToString().Length == 0)
						{
							...
							if (range2 != null)
							{
								string ssName = worksheet2.get_Range("H" + range2.Row.ToString(), Type.Missing).Value2.ToString();
								ssName = ssName.Remove(0, 3);
								if (range2.Value2.ToString() == int_1.ToString())
								{
									int r = Convert.ToInt32(worksheet.get_Range("I" + range.Row.ToString(), Type.Missing).Value2);
									EnumerableRowCollection<DataRow> source = from row in this.fhySxofeg2.AsEnumerable()
									where row.Field("TP") == ssName && (int)row.Field("tr") == r
									select row;
									double value = (source.Count<DataRow>() > 0) ? Math.Round((double)source.First<DataRow>().Field("tok") * 0.61661008749452029, 1) : 0.0;
									EnumerableRowCollection<DataRow> source2 = from row in this.dataTable_0.AsEnumerable()
									where row.Field("TP") == ssName
									select row;
		...
		private void method_7(Worksheet worksheet_1)
		{
			...
			IEnumerable<string> enumerable = from row in equipment.AsEnumerable()
			group row by row.Field("tp") into g
			select g.Key;
			int num = 0;
			using (IEnumerator<string> enumerator = enumerable.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					string substation = enumerator.Current;
					Range range = this.method_1(worksheet_1, "H", substation);
					if (range != null)
					{
						EnumerableRowCollection<DataRow> enumerableRowCollection = from row in equipment.AsEnumerable()
						where row.Field("tp") == substation
						select row;
						int num2 = 0;
						foreach (DataRow dataRow_ in enumerableRowCollection)
						{
		...
		private void hypeanaulB_Click(object sender, EventArgs e)
		{
		...
		private void button_5_Click(object sender, EventArgs e)
		{
			...
			var enumerable = from row in list
			group row by row.NameSubstation into g
			select new
			{
				Name = g.Key
			};
			...
			using (var enumerator2 = enumerable.GetEnumerator())
			{
				...
				{
					<>f__AnonymousType0<string> name = enumerator2.Current;
					Range range = this.method_1(this.worksheet_0, "H", name.Name);
					if (range != null)
					{
						foreach (TransfContract transfContract in from row in list
						where row.NameSubstation == name.Name
						select row)
						{
							for (int i = 0; i < 10; i++)
							{
		...
		private void button_13_Click(object sender, EventArgs e)
		{
			try
			{
				if (this.application_0 != null)
				{
					...
					DataTable transformer = GetData.GetTransformer();
					this.dataTable_0 = GetData.GetMaxPower();
					this.fhySxofeg2 = GetData.GetFactPower2014();
					IEnumerable<string> enumerable = from row in transformer.AsEnumerable()
					group row by row.Field("Расположение") into g
					select g.Key;
					int num = 0;
					using (IEnumerator<string> enumerator = enumerable.GetEnumerator())
					{
						...
						{
							string item = enumerator.Current;
							Range range3 = this.method_1(worksheet, "H", item);
							if (range3 != null)
							{
								EnumerableRowCollection<DataRow> enumerableRowCollection = from row in transformer.AsEnumerable()
								where row.Field("Расположение") == item
		...
		private void button_14_Click(object sender, EventArgs e)
		{
			try
			{
				if (this.application_0 != null)
				{
					...
					IEnumerable<string> enumerable = from row in connectedPower.AsEnumerable()
					group row by row.Field("SUBNAME") into g
					select g.Key;
					int num = 0;
					using (IEnumerator<string> enumerator = enumerable.GetEnumerator())
					{
						...
						while (enumerator.MoveNext())
						{
							...
							for (int i = 1; i < 9000; i++)
							{
								...
								if (range3 != null && range3.Value2 != null && range3.Value2.ToString() == item)
								{
									range3 = worksheet.get_Range("H" + i, Type.Missing);
									IL_19C:
									if (range3 != null && range3.get_Value(Type.Missing) != null)
									{
										using (IEnumerator<DataRow> enumerator2 = (from row in factPower.AsEnumerable()
										where row.Field("TP") == item
										...
										EnumerableRowCollection<DataRow> enumerableRowCollection = from row in connectedPower.AsEnumerable()
										where row.Field("SUBNAME") == item
										select row;
										int num2 = 0;
			...
			try
			{
				...
				using (IEnumerator<string> enumerator = enumerable.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						...
						if (range3 != null)
						{
							foreach (DataRow dataRow in from row in equipment.AsEnumerable()
							where row.Field("TP") == item
							select row)
							{
		...
		private void button_16_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				using (List<Company>.Enumerator enumerator = list.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						Company company = enumerator.Current;
						if (company.ShortName != "")
						{
							string[] array = (from row in substationNames.AsEnumerable()
							where row.Field("Name").IndexOf(company.ShortName) != -1
							select row.Field("Substation")).ToArray<string>();
							if (array != null)
							{
								num++;
								string[] array2 = array;
										...
										if (range != null && range.Value2 != null && range.Value2.ToString() == b)
										{
											...
											if (range != null)
											{
												...
												if (range2 != null && range2.get_Value(Type.Missing).ToString() == "+ Добавить смежную организацию")
												{
													...
													this.application_0.Application.Run("modServiceModule.UNPROTECT_SHEET", this.workbook_0.ActiveSheet, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
													((Range)range2.get_Offset(-1, 0).Cells.get__Default(1, 1)).set_Value(Type.Missing, company.FullNameOrg);
													this.application_0.Application.Run("modServiceModule.PROTECT_SHEET", this.workbook_0.ActiveSheet, true, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
												}
											}
											i++;
		...
		private void method_17()
		{
			...
			this.hypeanaulB.Location = new Point(124, 260);
			this.hypeanaulB.Name = "button3";
			this.hypeanaulB.Size = new Size(25, 23);
			this.hypeanaulB.TabIndex = 4;
			this.hypeanaulB.Text = "Расположение";
			this.hypeanaulB.UseVisualStyleBackColor = true;
			this.hypeanaulB.Click += this.hypeanaulB_Click;
			this.button_2.Location = new Point(155, 260);
			this.button_2.Name = "button4";
			this.button_2.Size = new Size(25, 23);
			this.button_2.TabIndex = 5;
			this.button_2.Text = "Доп. оборуд.";
			this.button_2.UseVisualStyleBackColor = true;
			this.button_3.Location = new Point(62, 289);
			this.button_3.Name = "button5";
			this.button_3.Size = new Size(25, 23);
			this.button_3.TabIndex = 6;
			this.button_3.Text = "Доп. трансф.";
			this.button_3.UseVisualStyleBackColor = true;
			this.button_4.Location = new Point(93, 289);
			this.button_4.Name = "button6";
			this.button_4.Size = new Size(25, 23);
			this.button_4.TabIndex = 7;
			this.button_4.Text = "По умолчанию";
			this.button_4.UseVisualStyleBackColor = true;
			this.button_4.Click += this.button_4_Click;
			this.button_5.Location = new Point(124, 289);
			this.button_5.Name = "button7";
			this.button_5.Size = new Size(25, 23);
			this.button_5.TabIndex = 8;
			this.button_5.Text = "Пр. мощ. договор";
			...
			this.button_6.Location = new Point(155, 289);
			this.button_6.Name = "button8";
			this.button_6.Size = new Size(25, 23);
			this.button_6.TabIndex = 10;
			this.button_6.Text = "Доп. выключатели";
			this.button_6.UseVisualStyleBackColor = true;
			this.button_7.Location = new Point(62, 318);
			this.button_7.Name = "button9";
			this.button_7.Size = new Size(25, 23);
			this.button_7.TabIndex = 11;
			this.button_7.Text = "Трансф. 0,4";
			this.button_7.UseVisualStyleBackColor = true;
			this.button_8.Location = new Point(93, 318);
			this.button_8.Name = "button10";
			this.button_8.Size = new Size(25, 23);
			this.button_8.TabIndex = 12;
			this.button_8.Text = "Трансф. пуст. \"В\"";
			...
			this.button_9.Location = new Point(124, 318);
			this.button_9.Name = "button11";
			this.button_9.Size = new Size(25, 23);
			this.button_9.TabIndex = 14;
			this.button_9.Text = "Стоим. трансф.";
			this.button_9.UseVisualStyleBackColor = true;
			this.button_9.Click += this.button_9_Click;
			this.button_10.Location = new Point(155, 318);
			this.button_10.Name = "button12";
			this.button_10.Size = new Size(25, 23);
			this.button_10.TabIndex = 15;
			this.button_10.Text = "Удаление договора";
			this.button_10.UseVisualStyleBackColor = true;
			this.button_10.Click += this.button_10_Click;
			this.button_11.Enabled = false;
			this.button_11.Location = new Point(201, 143);
			this.button_11.Name = "button13";
			this.button_11.Size = new Size(181, 23);
			this.button_11.TabIndex = 16;
			this.button_11.Text = "Удаление подстанции (не хоз.)";
			this.button_11.UseVisualStyleBackColor = true;
			this.button_11.Click += this.button_11_Click;
			this.button_12.Enabled = false;
			this.button_12.Location = new Point(10, 85);
			this.button_12.Name = "button14";
			this.button_12.Size = new Size(185, 23);
			this.button_12.TabIndex = 17;
			this.button_12.Text = "Подстанции / расположение";
			this.button_12.UseVisualStyleBackColor = true;
			this.button_12.Click += this.button_12_Click;
			this.button_13.Enabled = false;
			this.button_13.Location = new Point(10, 114);
			this.button_13.Name = "button15";
			this.button_13.Size = new Size(185, 23);
			this.button_13.TabIndex = 18;
			this.button_13.Text = "Трансформаторы";
			this.button_13.UseVisualStyleBackColor = true;
			this.button_13.Click += this.button_13_Click;
			this.button_14.Enabled = false;
			this.button_14.Location = new Point(10, 143);
			this.button_14.Name = "button16";
			this.button_14.Size = new Size(185, 23);
			this.button_14.TabIndex = 19;
			this.button_14.Text = "Договор на тех. присоед.";
			this.button_14.UseVisualStyleBackColor = true;
			this.button_14.Click += this.button_14_Click;
			this.button_15.Enabled = false;
			this.button_15.Location = new Point(201, 85);
			this.button_15.Name = "button17";
			this.button_15.Size = new Size(181, 23);
			this.button_15.TabIndex = 20;
			this.button_15.Text = "Оборудование";
			this.button_15.UseVisualStyleBackColor = true;
			this.button_15.Click += this.button_15_Click;
			this.button_16.Enabled = false;
			this.button_16.Location = new Point(201, 114);
			this.button_16.Name = "button18";
			this.button_16.Size = new Size(181, 23);
			this.button_16.TabIndex = 21;
			this.button_16.Text = "Границы балансовой принадл.";
			this.button_16.UseVisualStyleBackColor = true;
			this.button_16.Click += this.button_16_Click;
			this.button_17.Location = new Point(12, 12);
			this.button_17.Name = "btnOpenFile";
			this.button_17.Size = new Size(103, 23);
			this.button_17.TabIndex = 22;
			this.button_17.Text = "Открыть файл";
			...
			this.label_2.Text = "Имя файла";
			this.button_18.Location = new Point(305, 212);
			this.button_18.Name = "btnClose";
			this.button_18.Size = new Size(75, 23);
			this.button_18.TabIndex = 24;
			this.button_18.Text = "Закрыть";
			...
			base.Controls.Add(this.hypeanaulB);
