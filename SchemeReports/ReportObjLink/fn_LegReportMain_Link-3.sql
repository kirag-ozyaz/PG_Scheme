...
/****** Object:  UserDefinedFunction [dbo].[fn_LegReportMain_Link-3]    Script Date: 09.12.2020 0:05:20 ******/
SET ANSI_NULLS ON
GO
...
- добавил признак нормальной (псевдо) привязки к схеме - Normal
*/
...
-- legal
	...
	SELECT DISTINCT
		...
		ds.Quantity, 
		isnull(vals.idSchmObj, 0) as idSchmObj,
		vals.ValueKWT,
...
--	LEFT JOIN tAbnDoc_List lst ON lst.idAbn = abn.id
	LEFT JOIN tAbnDoc_Dogovor dog ON dog.idList = dl.id
--	LEFT JOIN tG_Dog_Obj obj ON obj.idDog = dog.id
		...
		Left join (select meterALL.PointID, meterALL.idAbnObj, meterALL.idSchmObj , meterALL.Number, meterALL.MeterType, meterALL.Location, meterALL.PointName, isnull(valMeter.DateAct,@DateBegin) DateAct, valMeter.Name TypeAct, valMeter.ValueKWT, valMeter.SizeKWT 
                         , meterALL.DateCheck , meterAll.Interval, meterAll.NameTrans, meterAll.TypeTrans, meterAll.DateCheckTrans, meterall.[DateBeginTrans], meterAll.IntervalTrans, meterAll.DateBegin, meterAll.Normal
			from 
              ...
              FROM            vG_MeterValAll
				 ...
				 FULL OUTER JOIN
						 ...
						 (select  meter.PointID, isnull(link2.idSchmObj,0) idSchmObj ,meter.Number,meter.[MeterType],meter.Location,meter.[PointName], meter.IDZone, rowid1, rowid2, isnull(meter.idAbnObj, link2.idAbnObj) idAbnObj
								 ,meter.DateCheck,meter.Interval, meter.NameTrans, meter.TypeTrans, meter.DateCheckTrans, meter.[DateBeginTrans], meter.IntervalTrans, meter.DateBegin, link2.Normal
                                  from
									...
									from tL_SchmAbn -- привязка к схеме
										...
										left join tSchm_ObjList obj on obj.id = tL_SchmAbn.idSchmObj
										where tAbn.typeAbn in (207,230,231) and obj.deleted = 0
									) link2 
									full outer join 
	...
	WHERE (d.dtPay between @DateBegin and @DateEnd) 
		...
		-- отсекаем контрагентов у которых договор закрыт до начала периода
        and ((dog.FlagBreak = 0) or (dog.FlagBreak <> 0 and dog.DateBreak > @dateBegin))
...
select 
		...
		isnull(link3.idSchmObj, 0) as idSchmObj,
        ...
        , 0 Normal -- нормальное присоединение в бытовом секторе нет (по умолчанию false)
from 
...
from vPointRegLast p --tPoint p
	...
	left join ( select ue.idPoint , ue.MonthDeb , sum (ue.[Itog] ) as Itog ,
                     (select sum(KWT) from tPrv_Debitorka
                      where Ok=1 and idPoint = ue.idPoint and MonthRasch = ue.MonthDeb
                      group by idPoint,MonthRasch) [ItogUl]-- sum (ul.KWT ) as ItogUl
...
from tL_SchmAbn -- привязка к схеме
	...
	where obj.deleted = 0 and abn.typeAbn = 206 and ( abn.isActive = 1 or ( abn.isActive = 0 and abn.DateChange > @DateEnd ))		
) link3 
on link3.idAbn = valMeter.idAbn and link3.idAbnObj = valMeter.idAbnObj and (link3.[Day] = valmeter.[dtPay]) 
-- у физиков не попадают активные абоненты у которых нет объемов
-- outer
