...
--	  and abnObj.isActive = 1 
	  and abnObj.deleted = 0
      and lst.deleted = 0 
--	  and lst.isActive <> 0 
	  and lst.DocDate < @dt
	  and ( dog.FlagBreak = 0 or (FlagBreak <> 0 and DateBreak > @dt) ) 
...
abnAll (idAbn, codeAbonent, docAbn, nameAbn, nameAbnFull, typeAbn, typeAbnName, legalAddress, postAddress, idAbnObj, nameObj, [address], idCity, City, idStreet, StreetName, House,
                    idCategory, CategoryName, idTariff, TarifName, TariffNameSocr, powerset, powerFact, sumkwt, Consumer, consumerName, contact, idAbnNet, nameAbnNet, abnServicing, CountMeter, CommentODS, cStatus, idMap) as
(     
