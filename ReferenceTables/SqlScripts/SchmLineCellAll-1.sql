...
(данный вариант выполняется 3 сек.)
...
/*44191*/
select cell.id, cell.idParent, cell.name, cell.typeCodeId, 
		case 
			when clCell.ParentKey like ';SCM;Cell;%' then (select top 1 fullname from cellall where id = cell.id) 
			when clCell.ParentKey like ';SCM;Line;'  then clCell.socrname + '-' + cell.name 
			else ''
		end fullname
from tSchm_ObjList AS cell
	...
	left join tr_classifier clSub on clSub.id = sub.typecodeid
where ( ( clCell.ParentKey like ';SCM;Cell;%' and exists(select top 1 fullname from cellall where id = cell.id)) or clCell.ParentKey like ';SCM;Line;' )
