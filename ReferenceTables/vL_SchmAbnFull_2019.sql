...
where 
 abnObj.typeAbn in (207,231,230) 
	and abnObj.isActive = 1 and abnObj.deleted = 0
	  ...
	  and ((charindex('нулевой ', cTariff.name) = 0 and  objReg.idStatus in (241, 242, 1293))  -- потребители (Рассч. по дог.пер., Рассч. по дог. э/с, Непосредственное управление)
					or (charindex('нулевой ', cTariff.name) = 1 and cStatus.id = 2350)) -- вход в сетевую (там подстанции) (2350 = Вход)
					-- если Переток и нулевой тариф - то абонент не наш
					--  если нулевой тариф и Непосредственное управление <== чей абонент
...
abnAll (idAbn, codeAbonent, docAbn, nameAbn, nameAbnFull, typeAbn, typeAbnName, legalAddress, postAddress, idAbnObj, nameObj, [address], idCity, City, idStreet, StreetName, House,
                    idCategory, CategoryName, idTariff, TarifName, TariffParentKey, TariffNameSocr, powerset, powerFact, sumkwt, idConsumerGroup, ConsumerGroupName, Consumer, consumerName, contact, idAbnNet, nameAbnNet, abnServicing, CountMeter, CommentODS, cStatus, idMap) as
(     
