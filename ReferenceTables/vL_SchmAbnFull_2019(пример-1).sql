...
abnLegal (idAbn, codeAbonent, docAbn, nameAbn, nameAbnFull, typeAbn, typeAbnName, legalAddress, postAddress, idAbnObj, nameObj, [address], KladrObjid, kladrObjName, idStreet, StreetName, House,
                    idCategory, CategoryName, idTariff, TarifName, TariffNameSocr, powerset, powerFact, sumKwt, Consumer, consumerName, contact, idAbnNet, abnNet, abnServicing, CountMeter, CommentODS, cStatus) as
(
...
where 
 abnObj.typeAbn in (207,231,230) 
	and abnObj.isActive = 1 and abnObj.deleted = 0
	  ...
	  and (charindex('нулевой ', cTariff.name) = 0 or (charindex('нулевой ', cTariff.name) = 1 and cStatus.id = 2350)) -- вход
...
abnFL (idAbn, codeAbonent, docAbn, nameAbn, nameAbnFull, typeAbn, typeAbnName, legalAddress, postAddress, idAbnObj, nameObj, [address], KladrObjid, kladrObjName, idStreet, StreetName, House,
				category, categoryName, idTariff, TarifName, TariffNameSocr, powerset, powerFact, sumKwt, Consumer, consumerName, contact, idAbnNet, abnNet, abnServicing, CountMeter, CommentODS, cStatus) as
(
...
-- select * from abnAll
select aaa.idAbn	,CodeAbonent,	nameAbn	,TypeAbn	,idAbnObj	,NameObj,	CommentODS,	CategoryName	,TarifName	,Voltage,	PowerSet	,CountPoint,	idAbnNet, rowid, countidobj, CountMeter, idSchmObj, cStatus
from
(
select
		abn2.*,
	 ...
	 --and abn2.idSchmObj = 787699
	  --and abn2.idSchmObj in (787699,787702,787700,1183116)
 ) aaa
