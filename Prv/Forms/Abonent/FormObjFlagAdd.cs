...
namespace Prv.Forms.Abonent
{
    public partial class FormObjFlagAdd : FormLbr.FormBase
    {
        ...
        {
            this.SelectSqlData(dsPrv, dsPrv.vPrv_ObjFlagsName, true, " order by Name");
        }
        ...
        private void buttonFlagAdd_Click(object sender, EventArgs e)
        {
            ...
            DataSql.SqlDataConnect sql = new DataSql.SqlDataConnect();
              ...
              DataRow r = dsPrv.tAbnObjFlags.NewRow();
            r["idAbnObj"]= idObj;
            r["idFlag"] = comboBoxFlagsName.SelectedValue;
            r["Status"] = 1 ;
            r["Date"] = dateTimePickerBegin.Value;
            r["Deleted"] = 0;
            dsPrv.tAbnObjFlags.Rows.Add(r);

            int idFlag = this.InsertSqlDataOneRow(dsPrv, dsPrv.tAbnObjFlags);
            if (idFlag > 0)
            {
