...
namespace SchemeCtrl2.DrawTool
{
	public class LabelTool : DrawObjectBase
	{
		...
		public override RectangleF GetBoundingRect(CanvasControl canvas, bool inflate)
		{
			...
			double x = (double)Math.Min(rectangleText.Left, rectangleText.Right);
			double y = (double)Math.Min(rectangleText.Top, rectangleText.Bottom);
			double width = (double)Math.Abs(rectangleText.Left - rectangleText.Right);
			double height = (double)Math.Abs(rectangleText.Top - rectangleText.Bottom);
			RectangleF result = new RectangleF((float)x, (float)y, (float)width, (float)height);
			if (inflate)
			{
		...
		public override bool DeleteSql()
		{
			...
			bool result = new SqlDataCommand(this.Layer.Parent.SqlSettings).DeleteSqlDataById(this.Layer.Parent.dsScheme.tSchm_Label, this.IdBase);
			this.IdBase = 0;
			return result;
		}
		...
		public override bool ShowPropertyForm(Point location)
		{
			...
			}.ShowDialog() == DialogResult.OK)
			{
				...
				{
					this.Layer.Parent.ProxySingleton.NodeChange(this.IdBase, flag ? 1 : 3, this.Text, this.Location.Point, base.UseSettingColor ? Color.Empty : this.Color, Class18.smethod_3());
				}
				return true;
			}
			return false;
