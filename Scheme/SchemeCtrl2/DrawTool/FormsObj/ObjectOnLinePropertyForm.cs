	...
	public partial class ObjectOnLinePropertyForm : Form
	{
		...
		public ObjectOnLinePropertyForm(ObjectOnLine objectOnLine, SQLSettings sqlSett)
		{
			...
			if (objectOnLine.Layer != null && objectOnLine.Layer.Parent != null)
			{
				...
				{
					this.qFxbcoyxuw.Enabled = false;
				}
				if (objectOnLine.Layer.Parent.Mode == eCanvasEditingMode.Dispatcher && objectOnLine.MappingState)
				{
					this.qFxbcoyxuw.Enabled = true;
				}
				this.checkBoxNormal.Enabled = (this.panel2.Enabled = (this.panel3.Enabled = (this.checkBoxSetComplexSwitchgear.Enabled = (this.textBoxName.Enabled = (this.panelOnOff.Enabled = !objectOnLine.LimitedEditing)))));
			}
			...
			if (typeSwitch <= TypeSwitch.FuseTool)
			{
				if (typeSwitch <= TypeSwitch.OutsideLineDisconnectorTool)
				{
			...
			else if (typeSwitch <= TypeSwitch.RPSTool)
			{
				if (typeSwitch == TypeSwitch.VoltageTransformerTool)
				{
					this.Text = "Свойство трансформатора напряжения";
					this.panelOnOff.Visible = false;
					return;
				}
				if (typeSwitch != TypeSwitch.RPSTool)
				{
					return;
				}
			...
			{
				if (typeSwitch != TypeSwitch.AmperageTransformer)
				{
					switch (typeSwitch)
					{
					...
					default:
						...
						this.checkBoxAPhase.Checked = ((MagneticStarterTool)this.objectOnLine_0).Boolean_0;
						this.checkBoxBPhase.Checked = ((MagneticStarterTool)this.objectOnLine_0).Boolean_1;
						this.checkBoxCPhase.Checked = ((MagneticStarterTool)this.objectOnLine_0).Boolean_2;
						break;
					}
					return;
				}
		...
		private void ObjectOnLinePropertyForm_FormClosing(object sender, FormClosingEventArgs e)
		{
			if (base.DialogResult == DialogResult.OK)
			{
				...
				{
					((MagneticStarterTool)this.objectOnLine_0).Boolean_0 = this.checkBoxAPhase.Checked;
					((MagneticStarterTool)this.objectOnLine_0).Boolean_1 = this.checkBoxBPhase.Checked;
					((MagneticStarterTool)this.objectOnLine_0).Boolean_2 = this.checkBoxCPhase.Checked;
				}
