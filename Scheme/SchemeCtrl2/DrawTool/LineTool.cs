	...
	public class LineTool : DrawObjectBase
	{
		...
		public LineTool(DataRow r, DrawingLayer layer)
		{
			...
			foreach (object obj5 in xmlNode2.ChildNodes)
			{
				...
				if (xmlAttribute != null)
				{
					...
					{
						bool flag = false;
						DataRow[] rowPoint = layer.Parent.dsScheme.vSchm_ObjListXml.Select(string.Concat(new object[]
						{
							"Id = ",
							num,
							" and typeSChema = ",
						...
						{
							rowPoint = layer.Parent.dsScheme.vSchm_ObjListXml.Select("Id = " + num);
							flag = true;
						}
						if (rowPoint.Length == 0)
						{
			...
			DataRow[] array = layer.Parent.dsScheme.tSchm_Relation.Select("Edge = " + this.IdBase.ToString());
			...
			foreach (DataRow dataRow in array)
			{
				...
				{
					DataRow[] array3 = layer.Parent.dsScheme.vSchm_ObjListXml.Select("Id = " + Convert.ToInt32(dataRow["SourceObj"]));
					if (array3.Length != 0)
					{
				...
				{
					DataRow[] array4 = layer.Parent.dsScheme.vSchm_ObjListXml.Select("Id = " + Convert.ToInt32(dataRow["DestObj"]));
					if (array4.Length != 0)
					{
					...
					else
					{
						...
						if (Convert.ToInt32(xmlNode6.Attributes[1].Value) != -1 && Convert.ToInt32(xmlNode6.Attributes[2].Value) != -1)
						{
							...
							foreach (object obj2 in xmlNode6.SelectNodes("objOnLine"))
							{
								...
								DataRow[] array5 = layer.Parent.dsScheme.vSchm_ObjListXml.Select(string.Format("Id = {0} and IdParent = {1} and Tag {2}", Convert.ToInt32(xmlNode7.Attributes[0].Value), couplingRelation2.Id, (num2 == -1) ? "IS NULL" : ("= " + num2)));
								if (array5.Length != 0)
								{
		...
		protected override void CallEndDrawEvent(DrawObjectBase sender)
		{
			...
			if (this.IdBase > 0)
			{
				...
				{
					new SqlDataCommand(this.Layer.Parent.SqlSettings).DeleteSqlData(this.Layer.Parent.dsScheme.tSchm_Xml, "where [ObjId] = " + this.IdBase + " AND TypeSchema=2");
				}
