...
{
	public partial class CanvasControl : UserControl, IDisposable
	{
        /// <summary>
        /// Происходит после добавления объекта на канвас
        /// </summary>
        ...
        /// <summary>
        /// Происходит при завершении рисования объекта
        /// </summary>
		[Category("Objects")]
        ...
        /// <summary>
        /// Происходит при показе контестного меню
        /// </summary>
        ...
        /// <summary>
        /// Происходит при закрытии контестного меню
        /// </summary>
...
#pragma warning restore CS0618 // Тип или член устарел
				...
				this.ProxySingleton.ProxyEvent += new Proxy_Singleton.ProxyEventHandler(this.ProxySingleton_ProxyEvent);
				this.ProxySingleton.Connect(ipaddress.ToString() + Environment.UserName + "_" + Process.GetCurrentProcess().Id.ToString(), netTcpBinding, this.endPoint, Class18.smethod_3());
				this.ProxySingleton.ProxyCallBackEvent += new Proxy_Singleton.ProxyCallBackEventHandler(this.ProxySingleton_ProxyCallBackEvent);
			}
			catch
			{
			}
		...
		private void ProxySingleton_ProxyEvent(object sender, bool e)
		{
			this.Connected = e;
			if (!this.checkServTimer.Enabled)
			{
				this.EnableTimerSafe(true);
			}
		}

		private void EnableTimerSafe(bool bool_12)
		{
		...
		{
			this.checkServTimer.Enabled = bool_12;
		}
		...
		public CanvasControl()
		{
			...
			this.checkServTimer = new System.Windows.Forms.Timer();
			...
			this.checkServTimer.Interval = 10000;
			this.checkServTimer.Tick += this.timer_0_Tick;
				...
				case eCommandType.move:
					...
					select o)
					{
						try
						{
							if (this.Connected)
							{
								this.ProxySingleton.NodeChange(drawObjectBase2.IdBase, 3, drawObjectBase2.Text, ((NodeTool)drawObjectBase2).Location.Point, drawObjectBase2.Color, Class18.smethod_3());
							}
						}
						catch (Exception ex)
						{
		...
		public bool SetObjectsInCenterScreen(List<int> idObjects, bool adaptZoom, CanvasControl.SetObjectsInCenterScreenOptions backlight)
		{
			...
			foreach (ICanvasLayer canvasLayer in this.Layers)
			{
				...
				{
					predicate = (C9__0 = ((DrawObjectBase dob) => idObjects.Contains(dob.IdBase)));
				}
		...
		public List<DrawObjectBase> GetHitObjects(UnitPoint point)
		{
			...
			foreach (ICanvasLayer canvasLayer in this.list_0.Reverse<ICanvasLayer>())
			{
				if (canvasLayer.Visible)
				{
					...
					if ((predicate = C9__0) == null)
					{
						predicate = (C9__0 = ((DrawObjectBase obj) => obj != this.drawObjectBase_0 && obj.Visible && obj.PointInObject(point)));
					}
		...
		public List<DrawObjectBase> GetHitObjects(CanvasControl canvas, RectangleF selection, bool anyPoint)
		{
			...
			foreach (ICanvasLayer canvasLayer in this.list_0)
			{
				if (canvasLayer.Visible)
				{
					...
					if ((predicate = C9__0) == null)
					{
						predicate = (C9__0 = ((DrawObjectBase obj) => obj.ObjectInRectangle(canvas, selection, anyPoint)));
					}
		...
		public bool ValidateToUnique(DrawObjectBase validateObj)
		{
			...
			foreach (ICanvasLayer canvasLayer in this.list_0)
			{
				...
				if ((predicate = C9__0) == null)
				{
					predicate = (C9__0 = ((DrawObjectBase objj) => objj.Parent == validateObj.Parent));
				}
		...
		public void CopyToImage()
		{
			...
			Func<DrawObjectBase, bool> C9__10=null;
			foreach (ICanvasLayer canvasLayer in this.Layers)
			{
				IEnumerable<DrawObjectBase> objects = ((DrawingLayer)canvasLayer).Objects;
				Func<DrawObjectBase, bool> predicate;
				if ((predicate = C9__10) == null)
				{
					predicate = (C9__10 = ((DrawObjectBase obj) => obj != null && obj.ObjectInRectangle(this, rectUnit, true)));
				}
		...
		private bool IsLockMonopoly()
		{
			...
			{
				if (sqlDataReader["ObjName"].ToString() == this.dsScheme.tSchm_Lock.TableName)
				{
		...
		private bool LockMonopoly()
		{
			if (!this.IsLockMonopoly())
			{
				this.dsScheme.tSchm_Lock.Clear();
				this.dsScheme.tSchm_Lock.NewRow();
				this.dsScheme.tSchm_Lock.Rows.Add(new object[0]);
				new SqlDataCommand(this.SqlSettings).InsertSqlData(this.dsScheme, this.dsScheme.tSchm_Lock);
				this.dsScheme.tSchm_Lock.Clear();
				SqlDataConnect sqlDataConnect = new SqlDataConnect();
				sqlDataConnect.OpenConnection(this.SqlSettings);
				this.sqlTransaction = sqlDataConnect.Connection.BeginTransaction();
				...
				}).Fill(this.dsScheme, this.dsScheme.tSchm_Lock.TableName);
		...
		public DrawObjectBase FindObjectById(int id)
		{
			...
			foreach (ICanvasLayer canvasLayer in this.Layers)
			{
				...
				{
					IEnumerable<DrawObjectBase> objects = canvasLayer.Objects;
					Func<DrawObjectBase, bool> predicate = null;
					if ((predicate = C9__0) == null)
					{
						predicate = (C9__0 = ((DrawObjectBase o) => o.IdBase == id));
					}
		...
		private void method_57(int int_9)
		{
			...
			if ((predicate = C9__0) == null)
			{
				predicate = (C9__0 = ((DrawObjectBase l) => l.GetType() == typeof(MeasurementLabelTool) && l.Parent != null && l.Parent.GetType() == typeof(ShinaTool) && l.Parent.IdBase == int_9));
			}
		...
		private void toolMenuItemStateDel_Click(object sender, EventArgs e)
		{
			...
			if (contextMenu.Tag != null && contextMenu.Tag is ObjectOnLine)
			{
				...
				if (state2 == SchemeCtrl2.DrawTool.States.break_down || state2 == SchemeCtrl2.DrawTool.States.off)
				{
					...
					sqlDataCommand.SelectSqlData(this.dsScheme.tP_CabOperation, true, " where idObjList = " + branch.Parent.IdBase.ToString() + " order by DateDecommissioning desc", null, false, 0);
					if (state.Mapping.SearchObjectsOnLine(typeof(OffTool)).Count == 0 && state.Mapping.SearchObjectsOnLine(typeof(DamageTool)).Count == 0 && this.dsScheme.tP_CabOperation.Rows.Count > 0 && this.dsScheme.tP_CabOperation.Rows[0]["DateCommissioning"] == DBNull.Value)
					{
						...
						((DataSetScheme.Class72)this.dsScheme.tP_CabOperation.Rows[0]).DateCommissioning = dateCommissioning;
						sqlDataCommand.UpdateSqlData(this.dsScheme, this.dsScheme.tP_CabOperation);
					}
				}
				...
				try
				{
					...
					this.ProxySingleton.StateOnLine(branch.Parent.IdBase, branch.CouplingRelation.Id, num, delta, (int)state2, false, id, idBase, Class18.smethod_3());
				}
				catch
				{
				}
		...
		private void toolMenuItemSwitchOn_Click(object sender, EventArgs e)
		{
			...
			if (contextMenu.Tag != null && contextMenu.Tag is ObjectOnLine)
			{
				...
				if (this.SimulationMode == eSimulationMode.Online)
				{
					...
					try
					{
						this.ProxySingleton.ObjectOnLineOnChange(objectOnLine.IdBase, objectOnLine.On, Class18.smethod_3());
						goto IL_AF;
					}
					catch
					{
		...
		private void toolMenuItemSwitchLogDispatcher_Click(object sender, EventArgs e)
		{
			...
			ObjectOnLine objectOnLine = (ObjectOnLine)contextMenu.Tag;
			SqlDataCommand sqlDataCommand = new SqlDataCommand(this.SqlSettings);
			sqlDataCommand.SelectSqlData(this.dsScheme.vSchm_LogDispatcher, true, string.Concat(new string[]
			{
				" where IdObj = ",
			...
			this.SendMessage(objectOnLine, (obj != null) ? obj.ToString() : string.Empty, this.dsScheme.vSchm_LogDispatcher);
		}
		...
		private void method_65(object object_0, States states_0)
		{
			...
			if (contextMenu.Tag != null)
			{
				...
				{
					new SqlDataCommand(this.SqlSettings).SelectSqlData(this.dsScheme.tP_CabOperation, true, " where idObjList = " + lineTool.IdBase.ToString() + " and deleted = 0 order by DateDecommissioning desc", null, false, 0);
					if (lineToolState.Mapping.SearchObjectsOnLine(typeof(OffTool)).Count == 0 && lineToolState.Mapping.SearchObjectsOnLine(typeof(DamageTool)).Count == 0 && (this.dsScheme.tP_CabOperation.Rows.Count == 0 || this.dsScheme.tP_CabOperation.Rows[0]["DateCommissioning"] != DBNull.Value))
					{
...
#pragma warning disable CS0618 // Тип или член устарел
                        string id = Dns.GetHostByName(Dns.GetHostName()).AddressList[0].ToString() + Environment.UserName + "_" + Process.GetCurrentProcess().Id.ToString();
#pragma warning restore CS0618 // Тип или член устарел
                        this.ProxySingleton.StateOnLine(lineTool.IdBase, lineTool.SelectedBranch.CouplingRelation.Id, num, lineToolState.Mapping.Delta, (int)states_0, true, id, lineToolState.Mapping.IdBase, Class18.smethod_3());
					}
					catch
					{
					}
		...
		private void toolStripMenuItemDisconnectLine_Click(object sender, EventArgs e)
		{
			...
			Func<Branch, bool> C9__0;
			Func<Branch, bool> predicate;
			if ((predicate = C9__0) == null)
			{
				predicate = (C9__0 = ((Branch bb) => bb.Parent == obj));
			}
		...
		private void toolMenuItemLineLogDispatcher_Click(object sender, EventArgs e)
		{
			...
			SqlDataCommand sqlDataCommand = new SqlDataCommand(this.SqlSettings);
			sqlDataCommand.SelectSqlData(this.dsScheme.vSchm_LogDispatcher, true, string.Format(" where (IdObj in ({0}) or idLine in ({0})) and typeSchema = {1} order by dateLog desc", text, ((int)this.TypeCanvas).ToString()), null, false, 0);
			object obj = sqlDataCommand.CallSQLScalarFunction("dbo.fn_Schm_GetFullNameObjById", new string[]
			{
				lineTool.IdBase.ToString()
			});
			this.SendMessage(lineTool, (obj != null) ? obj.ToString() : string.Empty, this.dsScheme.vSchm_LogDispatcher);
		}
		...
		private void toolMenuItemLineLinkPassport_Click(object sender, EventArgs e)
		{
			...
			{
				new SqlDataCommand(this.sqlsettings_0).SelectSqlData(this.dsScheme.vSchm_CabSection, true, "", null, false, 0);
				decimal num = 0m;
				string text = string.Empty;
				SqlConnection sqlConnection = new SqlConnection(this.SqlSettings.GetConnectionString());
				...
				if (lineTool.CouplingRelations.Count == 1)
				{
					...
					IEnumerable<DataRow> enumerable = this.dsScheme.vSchm_CabSection.Select("idObjList = " + lineTool.IdBase);
		...
		private void toolMenuItemLabelDel_Click(object sender, EventArgs e)
		{
			if (this.list_1 != null)
			{
				foreach (DrawObjectBase drawObjectBase in this.list_1)
				{
					if (drawObjectBase is NodeTool)
					{
						this.ProxySingleton.NodeChange(drawObjectBase.IdBase, 2, null, PointF.Empty, Color.Empty, Class18.smethod_3());
					}
				}
		...
		public List<int> ApplyFilter()
		{
			...
			if (formCanvasFilterApply.ShowDialog() == DialogResult.OK)
			{
				...
				{
					text = text.Remove(text.Length - 1);
					new SqlDataCommand(this.sqlsettings_0).SelectSqlData(this.dsScheme.tSchm_ObjFilter, true, "where [idFilter] in (" + text + ")", null, false, 0);
					foreach (DataRow dataRow in this.dsScheme.tSchm_ObjFilter)
					{
		...
		public void SaveCurrentXmlDocument()
		{
			...
			sqlDataCommand.SelectSqlData(this.dsScheme.tSchm_Schemes, true, "where [Name]='" + this.string_1 + "'", null, false, 0);
			if (this.dsScheme.tSchm_Schemes.Rows.Count == 0)
			{
				DataRow dataRow = this.dsScheme.tSchm_Schemes.NewRow();
				dataRow["Name"] = this.string_1;
				dataRow["SchmDataXml"] = this.xmlDocument_0.InnerXml;
				this.dsScheme.tSchm_Schemes.Rows.Add(dataRow);
				sqlDataCommand.InsertSqlDataOneRow(this.dsScheme.tSchm_Schemes);
				return;
			}
			this.dsScheme.tSchm_Schemes.Rows[0]["SchmDataXml"] = this.xmlDocument_0.InnerXml;
			this.dsScheme.tSchm_Schemes.Rows[0].EndEdit();
			sqlDataCommand.UpdateSqlData(this.dsScheme, this.dsScheme.tSchm_Schemes);
		}
		...
		public void SaveToBase()
		{
			...
			this.dsScheme.tSchm_Schemes.Rows[0]["SchmDataXml"] = xmlDocument.InnerXml;
			this.dsScheme.tSchm_Schemes.Rows[0].EndEdit();
			sqlDataCommand.UpdateSqlData(this.dsScheme, this.dsScheme.tSchm_Schemes);
		}
		...
		public void LoadScheme()
		{
			...
			if (typeCanvas == eTypeCanvas.SchemeMain)
			{
				...
				this.dsScheme.vSchm_CabSection.Clear();
				this.dsScheme.tSchm_ObjList.Clear();
				this.dsScheme.vP_PassportDataReports.Clear();
			}
		}
		...
		{
			SqlDataCommand sqlDataCommand = new SqlDataCommand(this.sqlsettings_0);
			sqlDataCommand.SelectSqlData(this.dsScheme.vSchm_ObjListXml, false, " where TypeSchema = " + 1 + " and [Deleted] = 0 union all select *  from vSChm_ObjListXml s2 where s2.typeSchema = 2 and s2.id not in (select id from vSchm_ObjListxml where typeSchema = 1) and deleted = 0", null, false, 0);
			this.SendMessage("Объектов в базе: " + this.dsScheme.vSchm_ObjListXml.Rows.Count.ToString(), eMessageType.Information);
			sqlDataCommand.SelectSqlData(this.dsScheme.tSchm_Relation, true, "", null, false, 0);
			this.SendMessage("Связей в базе: " + this.dsScheme.tSchm_Relation.Rows.Count.ToString(), eMessageType.Information);
			this.method_71();
		...
		public void LoadScheme(RectangleTool subStation)
		{
			...
			new SqlDataCommand(this.sqlsettings_0).SelectSqlData(this.dsScheme.tSchm_ObjList, false, " where id = " + idBase.ToString(), null, false, 0);
			if (this.dsScheme.tSchm_ObjList.Rows.Count == 0)
			{
				return;
			}
			bool flag = false;
			foreach (object obj2 in Enum.GetValues(typeof(SchemeCtrl2.DrawTool.eTypeRectangleTool)))
			{
				...
				if ((int)this.dsScheme.tSchm_ObjList.Rows[0]["TypeCodeId"] == (int)eTypeRectangleTool)
				{
					flag = true;
				}
			}
		...
		{
			SqlDataCommand sqlDataCommand = new SqlDataCommand(this.sqlsettings_0);
			sqlDataCommand.SelectSqlData(this.dsScheme.tSchm_Label, true, "where Tag = " + int_9.ToString(), null, false, 0);
			sqlDataCommand.SelectSqlData(this.dsScheme.vSchm_ObjListXml, false, string.Concat(new string[]
			{
				" where (id = ",
				int_9.ToString(),
				" or idParent = ",
			...
			DataRow[] array = this.dsScheme.vSchm_ObjListXml.Select(string.Concat(new string[]
			{
				" typecodeId in (",
				676.ToString(),
				",",
			...
			if (array.Length != 0)
			{
				...
				text = text.Remove(text.Length - 1);
				sqlDataCommand.SelectSqlData(this.dsScheme.tSchm_Relation, false, " where Edge in (" + text + ")", null, false, 0);
				if (this.dsScheme.tSchm_Relation.Rows.Count > 0)
				{
					string text2 = "";
					...
					text = "";
					string text3 = "";
					foreach (object obj2 in this.dsScheme.tSchm_Relation.Rows)
					{
					...
					DataSetScheme @class = new DataSetScheme();
					DataRow[] array3 = this.dsScheme.vSchm_ObjListXml.Select(string.Concat(new string[]
					{
						" typecodeId in (",
						547.ToString(),
						",",
					...
					if (text4.Length > 0)
					{
						...
						sqlDataCommand.SelectSqlData(this.dsScheme.vSchm_CabSection, true, " where deleted = 0 and idObjList in (" + text4 + ")", null, false, 0);
						text4 = string.Empty;
					}
					...
					foreach (object obj3 in @class.tSchm_Relation.Rows)
					{
						...
						this.dsScheme.tSchm_Relation.ImportRow(dataRow4);
					...
					foreach (object obj4 in @class.tSchm_Relation.Rows)
					{
						...
						this.dsScheme.tSchm_Relation.ImportRow(dataRow5);
					}
					text = text.Remove(text.Length - 1);
					if (text5.Length > 0)
					{
					...
					if (text4.Length > 0)
					{
						...
						foreach (object obj5 in @class.vSchm_ObjListXml.Rows)
						{
							...
							this.dsScheme.vSchm_ObjListXml.ImportRow(row);
						}
						sqlDataCommand.SelectSqlData(@class.tSchm_Relation, false, string.Concat(new string[]
						{
							" where Edge in (",
						...
						foreach (object obj6 in @class.tSchm_Relation.Rows)
						{
							...
							this.dsScheme.tSchm_Relation.ImportRow(dataRow6);
					...
					foreach (object obj7 in @class.vSchm_ObjListXml.Rows)
					{
						...
						this.dsScheme.vSchm_ObjListXml.ImportRow(row2);
					}
					...
					foreach (object obj8 in @class.vSchm_ObjListXml.Rows)
					{
						...
						this.dsScheme.vSchm_ObjListXml.ImportRow(row3);
					}
				}
			}
		}
		...
		private void method_70()
		{
			...
			DataTable journalData = PassportData.GetJournalData(this.sqlsettings_0, typeCodeId);
			typeCodeId = new int[]
			{
				546
			};
			DataTable journalData2 = PassportData.GetJournalData(this.sqlsettings_0, typeCodeId);
			new XmlDocument();
			string text = "";
			...
			text = text.Remove(text.Length - 1);
			IEnumerable<DataRow> enumerable = this.dsScheme.vSchm_ObjListXml.Select(string.Concat(new object[]
			{
				"TypeCodeId in (",
				text,
				") and typeSchema = ",
			...
			if (enumerable.Count<DataRow>() == 0)
			{
				enumerable = this.dsScheme.vSchm_ObjListXml.Select(string.Concat(new object[]
				{
					"TypeCodeId in (",
					text,
					") and typeSchema = ",
								...
								if (!right.IsEmpty)
								{
									foreach (PointTool pointTool in lineCellTool.GetPointsList())
									{
										if (pointTool.GetType() != typeof(PointShinaTool))
										{
											...
											{
												if (drawingLayer.Parent.dsScheme.vSchm_ObjListXml.Select(string.Concat(new object[]
												{
													"Id = ",
													pointTool.IdBase,
													" and typeSChema = ",
						...
						else if (!r.IsNull("TypeSchema"))
						{
							...
							foreach (CouplingRelation couplingRelation in lineCellTool2.CouplingRelations)
							{
								if (couplingRelation.method_3().Count != 0)
								{
									...
									DataRow[] array = drawingLayer.Parent.dsScheme.vSchm_ObjListXml.Select(string.Concat(new object[]
									{
										"IdParent = ",
										couplingRelation.Id,
										" and TypeCodeId in (",
							...
							foreach (Branch branch2 in lineCellTool2.Branches)
							{
								foreach (DrawObjectBase drawObjectBase2 in branch2.ChildObjects)
								{
									if (drawObjectBase2.GetType() == typeof(TransformerTool))
									{
										...
										DataRow[] array5 = journalData.Select("id = " + transformerTool2.IdBase.ToString() + " and IdPassport is not null");
										if (array5.Length != 0)
										{
			...
			text = text.Remove(text.Length - 1);
			using (IEnumerator<DataRow> enumerator2 = ((IEnumerable<DataRow>)this.dsScheme.vSchm_ObjListXml.Select("  TypeCodeId in (" + text + ")")).GetEnumerator())
			{
				while (enumerator2.MoveNext())
				{
					DataRow r = enumerator2.Current;
					if (!r.IsNull("TypeSchema") && Convert.ToInt32(r["TypeSchema"]) == 1)
					{
						...
						DataRow[] array6 = drawingLayer.Parent.dsScheme.tSchm_Relation.Select("Edge = " + lineTool.IdBase.ToString());
						CouplingRelation couplingRelation2 = null;
						DataRow[] array = array6;
						int i = 0;
						while (i < array.Length)
						{
							DataRow dataRow7 = array[i];
							if (this.loadedPoints.ContainsKey((int)dataRow7["SourceObj"]))
							{
								goto IL_FE4;
							}
							DataRow[] array7 = drawingLayer.Parent.dsScheme.vSchm_ObjListXml.Select("Id = " + Convert.ToInt32(dataRow7["SourceObj"]));
							if (array7.Length != 0)
							{
								PointTool pointTool2 = new PointTool();
								pointTool2.IdBase = Convert.ToInt32(array7[0]["id"]);
							...
							{
								DataRow[] array8 = drawingLayer.Parent.dsScheme.vSchm_ObjListXml.Select("Id = " + Convert.ToInt32(dataRow7["DestObj"]));
								if (array8.Length == 0)
								{
									goto IL_1123;
								}
								PointTool pointTool3 = new PointTool();
								pointTool3.IdBase = Convert.ToInt32(array8[0]["id"]);
						...
						foreach (Branch branch4 in lineTool.Branches)
						{
							foreach (DrawObjectBase drawObjectBase3 in branch4.ChildObjects)
							{
								if (drawObjectBase3.GetType() == typeof(TextBranchTool))
								{
									...
									IEnumerable<DataRow> enumerable4 = this.dsScheme.vSchm_CabSection.Select("idObjList = " + lineTool.IdBase);
									if (enumerable4.Count<DataRow>() > 0)
									{
										...
										DataRow[] array9 = journalData2.Select("id = " + lineTool.IdBase.ToString());
										if (array9.Length != 0)
										{
											textBranchTool.passportDataRow = array9[0];
											continue;
										}
										continue;
										IL_1694:
									...
									if (drawObjectBase3.GetType() == typeof(TransformerTool))
									{
										...
										DataRow[] array10 = journalData.Select("id = " + transformerTool3.IdBase.ToString());
										if (array10.Length != 0)
										{
											transformerTool3.passportDataRow = array10[0];
										}
									}
								}
							}
					...
					else if (!r.IsNull("TypeSchema"))
					{
						...
						foreach (CouplingRelation couplingRelation4 in lineTool3.CouplingRelations)
						{
							...
							{
								DataRow[] array = drawingLayer.Parent.dsScheme.vSchm_ObjListXml.Select("IdParent = " + couplingRelation4.Id);
								for (int i = 0; i < array.Length; i++)
								{
									DataRow dr = array[i];
									if ((from obj in drawingLayer.Objects
						...
						foreach (Branch branch5 in lineTool3.Branches)
						{
							foreach (DrawObjectBase drawObjectBase4 in branch5.ChildObjects)
							{
								if (drawObjectBase4.GetType() == typeof(TextBranchTool))
								{
									...
									IEnumerable<DataRow> enumerable5 = this.dsScheme.vSchm_CabSection.Select("idObjList = " + lineTool3.IdBase);
									if (enumerable5.Count<DataRow>() > 0)
									{
										...
										DataRow[] array11 = journalData2.Select("id = " + lineTool3.IdBase.ToString());
										if (array11.Length != 0)
										{
											textBranchTool2.passportDataRow = array11[0];
											continue;
										}
										continue;
										IL_1ADC:
								...
								else if (drawObjectBase4.GetType() == typeof(TransformerTool))
								{
									...
									DataRow[] array12 = journalData.Select("id = " + transformerTool4.IdBase.ToString());
									if (array12.Length != 0)
									{
										transformerTool4.passportDataRow = array12[0];
									}
								}
							}
						}
			...
			foreach (object obj5 in this.dsScheme.tSchm_Label.Rows)
			{
		...
		public void LoadSchemeFromXML(XmlDocument doc1, bool useObjectsFromDB)
		{
			...
			foreach (object obj2 in xmlNodeList)
			{
				...
				foreach (object obj3 in xmlNode4.SelectNodes("Group"))
				{
					...
					if (useObjectsFromDB)
					{
						...
						DataRow[] array = this.dsScheme.tSchm_ObjList.Select("id = " + rectangleTool.IdBase.ToString());
						if (array.Length == 0)
						{
							continue;
						}
					...
					foreach (object obj4 in xmlNode5.SelectNodes("Shina"))
					{
						...
						if (useObjectsFromDB)
						{
							...
							DataRow[] array2 = this.dsScheme.tSchm_ObjList.Select("id = " + shinaTool.IdBase.ToString());
							if (array2.Length == 0)
							{
								continue;
							}
					...
					foreach (object obj6 in xmlNode5.SelectNodes("Cell"))
					{
						...
						if (useObjectsFromDB)
						{
							...
							DataRow[] array3 = this.dsScheme.tSchm_ObjList.Select("id = " + lineCellTool.IdBase.ToString());
							if (array3.Length == 0)
							{
								continue;
							}
						...
						foreach (object obj7 in xmlNode8.SelectNodes("Branch"))
						{
							...
							foreach (object obj8 in xmlNode9.SelectNodes("Switch"))
							{
								...
								if (!useObjectsFromDB || this.dsScheme.tSchm_ObjList.Select("id = " + Convert.ToInt32(xmlNode10.Attributes[0].Value)).Length != 0)
								{
				...
				foreach (object obj10 in xmlNode4.SelectNodes("Line"))
				{
					...
					if (useObjectsFromDB)
					{
						DataRow[] array4 = this.dsScheme.tSchm_ObjList.Select("id = " + lineTool.IdBase.ToString());
						if (array4.Length == 0)
						{
							continue;
						}
					...
					foreach (object obj11 in xmlNode12.SelectNodes("Branch"))
					{
						...
						foreach (object obj12 in xmlNode13.SelectNodes("Switch"))
						{
							...
							if (!useObjectsFromDB || this.dsScheme.tSchm_ObjList.Select("id = " + Convert.ToInt32(xmlNode14.Attributes[0].Value)).Length != 0)
							{
		...
		private void method_72()
		{
			...
			if (typeCanvas == eTypeCanvas.SchemeMain)
			{
				sqlDataCommand.SelectSqlData(this.dsScheme.vSchm_ObjListXml, false, " where TypeSchema = " + (int)this.TypeCanvas, null, false, 0);
			}
			List<int> list = new List<int>();
			List<DrawObjectBase> list2 = new List<DrawObjectBase>();
			...
			int num = 0;
			using (IEnumerator enumerator4 = this.dsScheme.vSchm_ObjListXml.Rows.GetEnumerator())
			{
				while (enumerator4.MoveNext())
				{
			...
			{
				sqlDataCommand.DeleteSqlDataById(this.dsScheme.tSchm_ObjList, id);
			}
		}
		...
		private System.Windows.Forms.Timer checkServTimer;
