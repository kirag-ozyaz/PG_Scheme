	...
	public class PointTool : DrawObjectBase
	{
		...
		public override TextFont TextProperty
		{
			get
			{
				...
				{
					LineTool lineTool = (LineTool)this.method_6();
					if (lineTool != null)
					{
		...
		public PointTool(DataRow dataRow_0) : base(dataRow_0)
		{
			this.Color = Color.Aqua;
			this.point = default(UnitPoint);
			this.ChangePoint += this.PointTool_ChangePoint;
		...
		protected void PointTool_ChangePoint(DrawObjectBase sender, PointToolEvetsArgs pointToolEvetsArgs_0)
		{
		}
		...
		public override bool PointInObject(UnitPoint uPoint)
		{
			...
			return Class88.smethod_4(this.point, float_, uPoint);
		}
		...
		public override void DrawPrint(CanvasControl canvas, Graphics graphics_0, bool isColor = false)
		{
			...
			using (new Pen(Color.Black, ConvertCanvas.ConvertToScreen(canvas, (double)base.Width)))
			{
				...
				graphics_0.FillEllipse(solidBrush, pointF.X - 3f * canvas.Zoom, pointF.Y - 3f * canvas.Zoom, 6f * canvas.Zoom, 6f * canvas.Zoom);
		...
		public override ISnapPoint SnapPoint(CanvasControl canvas, UnitPoint point, List<DrawObjectBase> otherobjs, Type[] runningsnaptypes, Type usersnaptype)
		{
			...
			if (Class88.smethod_4(this.Point, float_, point))
			{
				return new Class104(canvas, this, this.Point);
			}
			return null;
		}
		...
		public override void SaveToXML(XmlNode xmlNode, List<PointTool> pointList)
		{
			...
			XmlElement xmlElement = ownerDocument.CreateElement("point");
			this.Id == "";
		...
		public bool RemoveBranch(Branch branch)
		{
			...
			{
				((LineTool)branch.Parent).Amperages[this].OffAmperage();
			}
		...
		private DrawObjectBase method_6()
		{
		...
		public override string GetUpdateScriptXML(bool children, List<int> list_2)
		{
			if (this.couplingRelations.Count > 0)
			{
				if (this.IdBase <= 0)
				{
					base.Save(false, false);
				}
				this.UpdateXML();
				if (this.Layer == null)
				{
				...
				if (list_2.Contains(this.IdBase))
				{
					text = "";
				}
				else
				{
					list_2.Add(this.IdBase);
				}
				return text;
			}
			else
			{
				if (this.Parent != null)
				{
					return this.Parent.GetUpdateScriptXML(false, list_2);
				}
				return string.Empty;
			}
		}
		...
		protected List<Branch> list_1 = new List<Branch>();
