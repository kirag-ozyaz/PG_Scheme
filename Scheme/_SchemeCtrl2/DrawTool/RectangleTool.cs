...
namespace SchemeCtrl2.DrawTool
{
	...
	public class RectangleTool : DrawObjectBase
	{
		...
		public eTypeRectangleTool TypeRectnagleTool
		{
			...
			set
			{
				if (this.typeRectangle != value)
				{
					this.ChangeTypeRectangleTool(this, new EventArgs());
					this.typeRectangle = value;
				}
			}
		}
		...
		public RectangleTool()
		{
			...
			this.ResizeRectangle += this.method_6;
		}

		public RectangleTool(DataRow dataRow_0, DrawingLayer layer) : base(dataRow_0)
		{
			this.TextProperty.Visible = false;
			this.ChangeTypeRectangleTool += this.RectangleTool_ChangeTypeRectangleTool;
			this.ResizeRectangle += this.method_6;
			...
			this.TypeRectnagleTool = (eTypeRectangleTool)Convert.ToInt32(dataRow_0["TypeCodeId"]);
			...
			if (xmlNode2 != null)
			{
				...
				{
					automaticEntryProvisionTool.Save(false, true);
				}
				flag = true;
			}
			...
			if (xmlNode2 != null)
			{
				...
				if (!layer.Parent.Connected)
				{
					specialLockingTool.Save(false, true);
				}
				flag = true;
			}
			...
			if (xmlNode2 != null)
			{
				...
				if (!layer.Parent.Connected)
				{
					telemechanicsTool.Save(false, true);
				}
				flag = true;
			}
			...
			if (xmlNode2 != null)
			{
				...
				if (!layer.Parent.Connected)
				{
					telemechanicsProtectionTool.Save(false, true);
				}
				flag = true;
			}
			...
			if (xmlNode2 != null)
			{
				...
				if (!layer.Parent.Connected)
				{
					responsibleAbonentTool.Save(false, true);
				}
				flag = true;
			}
			if (flag)
		...
		public void RotateAboutThePoint(double angle, UnitPoint centerOfRotation)
		{
			...
			{
				UnitPoint unitPoint = new UnitPoint(this.rectangle.Location);
				...
				this.rectangle.Width = this.rectangle.Width + this.rectangle.Height;
				this.rectangle.Height = this.rectangle.Width - this.rectangle.Height;
				this.rectangle.Width = this.rectangle.Width - this.rectangle.Height;
				this.rectangle.Location = new PointF(Convert.ToSingle(value) - this.rectangle.Width, Convert.ToSingle(value2));
				flag = true;
			}
			if (!flag)
			{
		...
		public override void Copy(DrawObjectBase acopy)
		{
			...
			{
				this.rectangle = new RectangleF(((RectangleTool)acopy).Rectangle.Location, ((RectangleTool)acopy).Rectangle.Size);
		...
		public override void Move(UnitPoint offset, bool selectedMove)
		{
			...
			this.rectangle.X = this.rectangle.X + (float)offset.X;
			this.rectangle.Y = this.rectangle.Y + (float)offset.Y;
			foreach (DrawObjectBase drawObjectBase in this.ChildObjects)
			{
		...
		public override ISnapPoint SnapPoint(CanvasControl canvas, UnitPoint point, List<DrawObjectBase> otherobjs, Type[] runningsnaptypes, Type usersnaptype)
		{
			...
			for (int i = 0; i < this.NodeCount; i++)
			{
				...
				if (Class88.smethod_4(node, float_, point))
				{
					return new Class104(canvas, this, node);
				}
			}
			return null;
		}
		...
		public override RectangleF GetBoundingRect(CanvasControl canvas, bool inflate)
		{
			...
			double num2 = (double)Math.Min(this.rectangle.Left, this.rectangle.Right);
			double num3 = (double)Math.Min(this.rectangle.Top, this.rectangle.Bottom);
			double num4 = (double)Math.Abs(this.rectangle.Left - this.rectangle.Right);
			double num5 = (double)Math.Abs(this.rectangle.Top - this.rectangle.Bottom);
		...
		protected override RectangleF GetRectangleText(CanvasControl canvas)
		{
			...
			float num = (this.rectangle.Left < this.rectangle.Right) ? this.rectangle.Left : this.rectangle.Right;
			float num2 = (this.rectangle.Left > this.rectangle.Right) ? this.rectangle.Left : this.rectangle.Right;
			float num3 = (this.rectangle.Top < this.rectangle.Bottom) ? this.rectangle.Top : this.rectangle.Bottom;
			float num4 = (this.rectangle.Top > this.rectangle.Bottom) ? this.rectangle.Top : this.rectangle.Bottom;
			...
			if (this.TextProperty.Visible)
			{
				...
				{
					text = text + " " + this.SUBSCRIBER_TEXT;
				}
		...
		public override void Draw(CanvasControl canvas, RectangleF unitrect, eDrawMode drawMode = eDrawMode.Color)
		{
			...
			using (Pen pen = new Pen(this.Color, ConvertCanvas.ConvertToScreen(canvas, (double)base.Width)))
			{
				...
				PointF pointF = ConvertCanvas.ConvertToScreen(canvas, new UnitPoint(this.rectangle.Location));
				PointF pointF2 = ConvertCanvas.ConvertToScreen(canvas, new UnitPoint((double)this.rectangle.Right, (double)this.rectangle.Bottom));
					...
					if (rectangleF.Size.Width > 0f && rectangleF.Size.Height > 0f)
					{
						using (SolidBrush solidBrush = new SolidBrush(this.TextProperty.Color))
						{
							...
							if (this.IsSubscriber)
							{
								...
								SizeF size = canvas.Graphics.MeasureString(this.SUBSCRIBER_TEXT, font);
								...
								canvas.Graphics.DrawString(this.SUBSCRIBER_TEXT, font, solidBrush, rectangleF2.Location);
		...
		public override void DrawPrint(CanvasControl canvas, Graphics graphics_0, bool isColor = false)
		{
			...
			graphics_0.DrawRectangle(pen, num, num2, num3 - num, num4 - num2);
		...
		{
			this.rectangle = new RectangleF(point.Point, new SizeF(0f, 0f));
		...
		public override bool PointInObject(UnitPoint point)
		{
			...
			if (Class88.smethod_4(unitPoint, num, point))
			{
				this.clickedPoint = point;
				return true;
			}
			if (Class88.smethod_4(unitPoint + new UnitPoint(unitPoint_.X - unitPoint.X, 0.0), num, point))
			{
				this.clickedPoint = point;
				return true;
			}
			if (Class88.smethod_4(unitPoint + new UnitPoint(0.0, unitPoint_.Y - unitPoint.Y), num, point))
			{
				this.clickedPoint = point;
				return true;
			}
			if (Class88.smethod_4(unitPoint_, num, point))
			{
				this.clickedPoint = point;
				return true;
			}
		...
		public override bool ObjectInRectangle(CanvasControl canvas, RectangleF rect, bool anyPoint)
		{
			...
			if (anyPoint)
			{
				...
				return Class88.smethod_0(unitPoint_, unitPoint_2, rect);
			}
		...
		public override void MoveHandleTo(UnitPoint pointU, int handleNumber)
		{
			...
			float num = this.rectangle.Left;
			float num2 = this.rectangle.Top;
			float num3 = this.rectangle.Right;
			float num4 = this.rectangle.Bottom;
			switch (handleNumber)
			{
			case 1:
				num = point.X;
			...
			this.rectangle = new RectangleF(num, num2, num3 - num, num4 - num2);
			UnitPoint unitPoint = new UnitPoint((double)num, (double)num2);
			UnitPoint unitPoint2 = new UnitPoint((double)this.rectangle.Right, (double)this.rectangle.Bottom);
			foreach (DrawObjectBase drawObjectBase in this.ChildObjects)
			{
			...
			if (unitPoint.X < (double)this.rectangle.Left)
			{
				this.rectangle.X = (float)unitPoint.X;
			}
			if (unitPoint.Y < (double)this.rectangle.Top)
			{
				this.rectangle.Y = (float)unitPoint.Y;
			}
			if (unitPoint2.X > (double)this.rectangle.Right)
			{
				this.rectangle.Width = (float)unitPoint2.X - this.rectangle.Left;
			}
			if (unitPoint2.Y > (double)this.rectangle.Bottom)
			{
				this.rectangle.Height = (float)unitPoint2.Y - this.rectangle.Top;
			}
			ResizeRectangleEventsArgs resizeRectangleEventsArgs_ = new ResizeRectangleEventsArgs(new RectangleF(num, num2, num3 - num, num4 - num2), handleNumber);
			this.ResizeRectangle(this, resizeRectangleEventsArgs_);
		}
		...
		public override void Normalize()
		{
			...
			float num3 = (this.rectangle.Top < this.rectangle.Bottom) ? this.rectangle.Top : this.rectangle.Bottom;
			float num4 = (this.rectangle.Top > this.rectangle.Bottom) ? this.rectangle.Top : this.rectangle.Bottom;
			this.rectangle = new RectangleF(num, num3, num2 - num, num4 - num3);
		}

		public override void EndDraw()
		{
			FormRectangleProperties formRectangleProperties = new FormRectangleProperties(this, this.Layer.Parent.SqlSettings);
			if (formRectangleProperties.ShowDialog() == DialogResult.OK)
			{
				return;
			}
		...
		public override void SaveToXML(XmlNode xmlNode, List<PointTool> pointList)
		{
			...
			XmlElement xmlElement = ownerDocument.CreateElement("Group");
		...
		public override List<DrawObjectBase> GetRecipientsPower()
		{
			...
			foreach (DrawObjectBase drawObjectBase in this.ChildObjects)
			{
				if (drawObjectBase.GetType() == typeof(ShinaTool) && ((ShinaTool)drawObjectBase).Amperage != null)
				{
					...
					{
						List<DrawObjectBase> recipientsPower2 = drawObjectBase.GetRecipientsPower();
						foreach (DrawObjectBase item in recipientsPower2)
						{
		...
		public LabelTool AddLabelTool(DataRow dataRow_0)
		{
			LabelTool labelTool = null;
			Type type = null;
			switch (Convert.ToInt32(dataRow_0["TypeLabel"]))
			{
			case 1:
			...
			XmlNode xmlNode = xmlDocument.SelectSingleNode("/Obj");
			labelTool.IdBase = Convert.ToInt32(dataRow_0["id"]);
			...
			{
				labelTool.Text = dataRow_0["Name"].ToString();
			}
		...
		private void method_6(RectangleTool rectangleTool_0, ResizeRectangleEventsArgs resizeRectangleEventsArgs_0)
		{
		}
		...
		private eTypeRectangleTool typeRectangle = eTypeRectangleTool.TP;

		private bool isSubscriber;

		private string SUBSCRIBER_TEXT = "АБ";
