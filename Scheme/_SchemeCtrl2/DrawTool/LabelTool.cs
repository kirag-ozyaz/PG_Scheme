	...
	public class LabelTool : DrawObjectBase
	{
		private DrawObjectBase Anckor
		{
			get
			{
				...
				if (this.bool_0 && this.Parent.Parent != null && this.Parent.Parent is RectangleTool)
				{
		...
		public LabelTool(DataRow dataRow_0) : base(dataRow_0)
		{
		...
		public static LabelTool GetLabelTool(DataRow dataRow_0)
		{
			LabelTool result = null;
			Type type = null;
			eTypeLabelTool eTypeLabelTool = (eTypeLabelTool)Convert.ToInt32(dataRow_0["TypeLabel"]);
		...
		public override void Move(UnitPoint offset, bool selectedMove)
		{
			...
			if (this.Anckor != null && this.Anckor.Layer != null && this.Anckor.Layer.Parent != null)
			{
				if (this.Anckor is LineTool)
				{
					LineTool lineTool = (LineTool)this.Anckor;
					...
					{
						UnitPoint unitPoint_ = Class88.smethod_5(branch3.Source.Point, branch3.Destination.Point, this.Location);
						float num3 = Class88.smethod_6(unitPoint_, this.unitPoint_0);
						PointTool pointTool = this.method_7(unitPoint_, branch3);
						bool flag2 = false;
						if (pointTool != null)
						{
							foreach (Branch branch4 in pointTool.Branches)
							{
								if (branch4.Parent != lineTool)
								{
									flag2 = true;
									break;
								}
							}
							float num4 = Class88.smethod_6(unitPoint_, pointTool.Point);
							if (num4 > num3)
							{
								num3 = num4;
							}
						}
						if (num3 < width && (pointTool == null || Class88.smethod_6(unitPoint_, pointTool.Point) < width))
						{
							flag = true;
							break;
						}
					...
					if (!flag)
					{
						...
						UnitPoint unitPoint_2 = Class88.smethod_5(branch.Source.Point, branch.Destination.Point, this.Location);
						if (num > width)
						{
							UnitPoint unitPoint = this.method_6(unitPoint_2, this.Location, width / num);
							this.unitPoint_0.X = unitPoint.X;
							this.unitPoint_0.Y = unitPoint.Y;
						}
						PointTool pointTool2 = this.method_7(unitPoint_2, branch);
						if (pointTool2 != null)
						{
							num = Class88.smethod_6(unitPoint_2, pointTool2.Point);
							if (num > width)
							{
								UnitPoint unitPoint2 = this.method_6(pointTool2.Point, this.Location, width / num);
				...
				else
				{
					...
					RectangleF boundingRect = this.Anckor.GetBoundingRect(this.Parent.Layer.Parent, false);
		...
		private UnitPoint method_6(UnitPoint unitPoint_1, UnitPoint unitPoint_2, float float_0)
		{
			UnitPoint result = default(UnitPoint);
			float num = 1f - float_0;
			float num2 = Class88.smethod_6(unitPoint_1, unitPoint_2);
			float num3 = num2 * num;
			...
			{
				double num4 = 1.0 / (Math.Sqrt(Math.Pow((unitPoint_1.Y - unitPoint_2.Y) / (unitPoint_1.X - unitPoint_2.X), 2.0) + 1.0) / (double)num3);
				if (unitPoint_1.X < unitPoint_2.X)
				{
					num4 *= -1.0;
				}
				double num5 = 1.0 / (Math.Sqrt(Math.Pow((unitPoint_1.X - unitPoint_2.X) / (unitPoint_1.Y - unitPoint_2.Y), 2.0) + 1.0) / (double)num3);
				if (unitPoint_1.Y < unitPoint_2.Y)
				{
					num5 *= -1.0;
				}
				result = new UnitPoint(unitPoint_2.X + num4, unitPoint_2.Y + num5);
			}
			return result;
		}

		private PointTool method_7(UnitPoint unitPoint_1, Branch branch_0)
		{
		...
		protected override RectangleF GetRectangleText(CanvasControl canvas)
		{
			...
			empty = new RectangleF(this.Location.Point, size);
			font.Dispose();
			return empty;
		}
		...
		public override bool PointInObject(UnitPoint point)
		{
			...
			if (Class88.smethod_4(left, num, point))
			{
				return true;
			}
			if (Class88.smethod_4(left + new UnitPoint(unitPoint.X - left.X, 0.0), num, point))
			{
				return true;
			}
			if (Class88.smethod_4(left + new UnitPoint(0.0, unitPoint.Y - left.Y), num, point))
			{
				return true;
			}
		...
		public override bool ObjectInRectangle(CanvasControl canvas, RectangleF rect, bool anyPoint)
		{
			...
			if (anyPoint)
			{
				...
				return Class88.smethod_0(unitPoint, unitPoint_, rect);
			}
		...
		public override bool DeleteSql()
		{
			...
			bool result = sqlDataCommand.DeleteSqlDataById(this.Layer.Parent.dataSetScheme_0.tSchm_Label, this.IdBase);
			this.IdBase = 0;
			return result;
		}
		...
		public override bool ShowPropertyForm(Point location)
		{
			...
			}.ShowDialog() == DialogResult.OK)
			{
				...
				if (this.Layer != null && this.Layer.Parent != null && this.Layer.Parent.Connected)
				{
					this.Layer.Parent.ProxySingleton.NodeChange(this.IdBase, flag ? 1 : 3, this.Text, this.Location.Point, base.UseSettingColor ? Color.Empty : this.Color);
				}
				return true;
			}
			return false;
		...
		{
			FormLabelToolProperties formLabelToolProperties = new FormLabelToolProperties(this, this.Layer.Parent.SqlSettings);
			if (formLabelToolProperties.ShowDialog() == DialogResult.OK)
			{
				return;
			}
		...
		protected override bool SaveToSql()
		{
			...
			if (this.IdBase > 0)
			{
				sqlDataCommand.SelectSqlData(dataSetScheme, dataSetScheme.tSchm_Label, true, " where id = " + this.IdBase.ToString());
				if (dataSetScheme.tSchm_Label.Rows.Count != 0)
				{
					if (dataSetScheme.tSchm_Label.Rows[0]["Name"].ToString() != this.Text)
					{
						dataSetScheme.tSchm_Label.Rows[0]["Name"] = this.Text;
					}
					if ((int)dataSetScheme.tSchm_Label.Rows[0]["TypeLabel"] != this.GetTypeObject())
					{
						dataSetScheme.tSchm_Label.Rows[0]["TypeLabel"] = this.GetTypeObject();
					}
					if (this.Parent != null)
					{
						if (dataSetScheme.tSchm_Label.Rows[0].IsNull("idParent") || (int)dataSetScheme.tSchm_Label.Rows[0]["idParent"] != this.Parent.IdBase)
						{
							dataSetScheme.tSchm_Label.Rows[0]["idParent"] = this.Parent.IdBase;
						}
					}
					else
					{
						dataSetScheme.tSchm_Label.Rows[0]["idParent"] = DBNull.Value;
					}
					if (this.Layer.Parent.IdRect > 0)
					{
						dataSetScheme.tSchm_Label.Rows[0]["Tag"] = this.Layer.Parent.IdRect;
					}
					this.UpdateXML();
					dataSetScheme.tSchm_Label.Rows[0]["XmlCoords"] = this.xmlCoords.InnerXml;
					dataSetScheme.tSchm_Label.Rows[0].EndEdit();
					return sqlDataCommand.UpdateSqlData(dataSetScheme, dataSetScheme.tSchm_Label);
				}
				DataRow dataRow = dataSetScheme.Tables["tSchm_Label"].NewRow();
				...
				dataSetScheme.Tables["tSchm_Label"].Rows.Add(dataRow);
				this.IdBase = sqlDataCommand.InsertSqlDataOneRow(dataSetScheme, dataSetScheme.Tables["tSchm_Label"]);
				if (this.IdBase <= 0)
				{
					return false;
				}
			...
			{
				DataRow dataRow2 = dataSetScheme.tSchm_Label.NewRow();
				...
				dataSetScheme.tSchm_Label.Rows.Add(dataRow2);
				this.IdBase = sqlDataCommand.InsertSqlDataOneRow(dataSetScheme, dataSetScheme.tSchm_Label);
				if (this.IdBase <= 0)
				{
					return false;
				}
