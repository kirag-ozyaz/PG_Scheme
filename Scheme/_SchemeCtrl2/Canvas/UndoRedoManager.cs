...
namespace SchemeCtrl2.Canvas
{
	public class UndoRedoManager
	{
		...
		private static Stack<UndoRedoManager.RegistrarOfChanges> stack_0 = new Stack<UndoRedoManager.RegistrarOfChanges>();

		private static Stack<UndoRedoManager.RegistrarOfChanges> stack_1 = new Stack<UndoRedoManager.RegistrarOfChanges>();
		...
		public delegate void CanUndoRedoEventHandler(UndoRedoManager.TypeAction typeAction_0, bool bool_0);
		...
		public class RegistrarOfChanges
		{
			...
			public RegistrarOfChanges(DrawObjectBase drawObjectBase_0, int numPoint, UnitPoint oldPoint, UnitPoint newpoint)
			{
				this.changedObject = drawObjectBase_0;
				this.int_0 = numPoint;
			...
			public RegistrarOfChanges(List<DrawObjectBase> drawObject, UndoRedoManager.ChangesMode changesMode_1)
			{
				this.changesMode_0 = changesMode_1;
			...
			public void Undo()
			{
				...
				case UndoRedoManager.ChangesMode.PropertyChanged:
					...
					{
						((Amperage)this.changedObject.GetType().GetProperty(this.nameChangedProperty).GetValue(this.changedObject, null)).OffAmperage();
						return;
					}
				...
				case UndoRedoManager.ChangesMode.Move:
				{
					...
					double double_ = unitPoint2.X - unitPoint.X;
					double double_2 = unitPoint2.Y - unitPoint.Y;
			...
			private void method_0(DrawObjectBase drawObjectBase_0)
			{
				...
				if (drawObjectBase_0.GetType() != typeof(LineTool))
				{
					if (drawObjectBase_0.GetType() != typeof(LineCellTool))
					{
						...
						{
							((ObjectOnLine)drawObjectBase_0).OnChanged += UndoRedoManager.canvasControl_0.method_33;
							goto IL_DA;
						}
						goto IL_DA;
					}
				}
				((LineTool)drawObjectBase_0).StateAdded += UndoRedoManager.canvasControl_0.method_32;
				((LineTool)drawObjectBase_0).StateRemoved += UndoRedoManager.canvasControl_0.method_31;
				IL_DA:
				drawObjectBase_0.ErrorEvent += UndoRedoManager.canvasControl_0.method_30;
				drawObjectBase_0.SelectedChanged += UndoRedoManager.canvasControl_0.method_45;
				SqlDataCommand sqlDataCommand = new SqlDataCommand(UndoRedoManager.canvasControl_0.SqlSettings);
				drawObjectBase_0.Deleted = !sqlDataCommand.UpdateSqlData(UndoRedoManager.canvasControl_0.dataSetScheme_0.tSchm_ObjList, " set [DELETED] = 0 ", " WHERE [ID] = " + drawObjectBase_0.IdBase.ToString());
				...
				if (drawObjectBase_0.GetType() != typeof(LineTool))
				{
					if (drawObjectBase_0.GetType() != typeof(LineCellTool))
					{
						if (drawObjectBase_0.GetType() == typeof(LineTool))
						{
							drawObjectBase_0.RecalcAmperage();
							return;
						}
						return;
					}
