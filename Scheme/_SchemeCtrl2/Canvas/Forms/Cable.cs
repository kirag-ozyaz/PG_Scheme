...
namespace SchemeCtrl2.Canvas.Forms
{
	public static class Cable
	{
		public static void InsertPassportCable(SQLSettings settings, int Cod, int idObjList)
		{
			...
			int int_ = (int)sqlDataCommand.CallSQLScalarFunction("dbo.fn_P_GetIdAbn", new string[]
			{
				dataRow["MontagSMU"].ToString()
			});
			int idPassportCable = Cable.InsertPassportCable(idOldPassport, settings, idObjList, now, number, state, name, region, sector, balance, inventoryNumber, yearLaying, yearExpl, yearRebuild, isummer, iwinter, voltage, buyer, trest, int_, -1, -1);
		...
		public static int InsertPassportCable(int idOldPassport, SQLSettings settings, int idObjList, DateTime date, string number, string state, string name, int region, string sector, int balance, string inventoryNumber, int yearLaying, int yearExpl, int yearRebuild, decimal Isummer, decimal Iwinter, decimal voltage, int buyer, int trest, int int_0, int owner, int service)
		{
			...
			Cable.smethod_5(settings, num, new List<Parameter>
			{
				...
				new Parameter(139, int_0.ToString()),
		...
		private static bool smethod_2(SQLSettings sqlsettings_0, int int_0, string string_0)
		{
			...
			return (int)sqlDataCommand.CallSQLScalarFunction("dbo.fn_P_ExistsNameClutch", new string[]
			{
				int_0.ToString(),
				string_0
			}) > 0;
		...
		private static DataTable smethod_10(SQLSettings sqlsettings_0, string string_0)
		{
			...
			{
				sqlDataConnect.OpenConnection(sqlsettings_0);
				SqlCommand selectCommand = new SqlCommand(string_0, sqlDataConnect.Connection);
		...
		private static int smethod_11(string num_depart)
		{
			if (num_depart != null)
			{
				if (num_depart == "1")
