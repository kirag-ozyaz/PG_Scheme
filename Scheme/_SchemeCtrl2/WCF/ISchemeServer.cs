...
namespace SchemeCtrl2.WCF
{
	...
	public interface ISchemeServer
	{
		...
		[OperationContract(IsOneWay = true, IsInitiating = false, IsTerminating = false)]
		void ObjectChanged(ObjectBaseServer objectBaseServer_0);

		[OperationContract(IsOneWay = true, IsInitiating = false, IsTerminating = false)]
		void ObjectOnLineOnChange(int idBase, bool bool_0);

		[OperationContract(IsOneWay = true, IsInitiating = false, IsTerminating = false)]
		void StateOnLine(int idBase, int idRelation, int numBranch, double delta, States state, bool bool_0, string string_0);
	}
}
