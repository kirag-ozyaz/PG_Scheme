...
namespace SchemeCtrl2.DrawTool.FormsObj
{
	public partial class FormRectangleProperties : Form
	{
		...
		{
			new SqlDataCommand(this.sqlsettings_0).SelectSqlData(this.dataTable_0, true, " where parentKey = ';SCM;PS;' and isgroup = 0 and deleted = 0", null, false, 0);
		...
		private void method_2()
		{
			if (this.cmbType.SelectedIndex >= 0)
			{
				...
				sqlDataCommand.SelectSqlData(this.dataTable_1, true, " where typeCodeId = " + ((int)this.cmbType.SelectedValue).ToString() + " order by name ", null, false, 0);
		...
		private void oOyAhwAlKY(object sender, EventArgs e)
		{
			this.method_2();
		}
