...
namespace EIS.Forms
{
    public partial class FormMain : FormBase, IPluginHost
    {
        ...
        public FormMain()
        {
            ...
            this.toolStripMenuItemJournalN.Visible = false;
            this.toolStripMenuItemJournalN.Visible = false;
        ...
        private void LoadConfig()
        {
            ...
            if (!eisSettings.IsShowFormConnect && new SqlDataConnect().CheckConnection(eisSettings.SqlSettings))
            {
                ...
                if ((!(this.SqlSettings.Autentification.ToUpper() == "WINDOWS")
                            ? this.SelectSqlData("tUser", false, "where Login = '" + this.SqlSettings.SqlUserConnect + "' AND Deleted = 0") : this.SelectSqlData("tUser", false, "where Login = SYSTEM_USER AND Deleted = 0")).Rows.Count > 0)
                    return;
            ...
            while (true)
            {
                ...
                //DataTable dataTable = new DataTable();
                if ((!(this.SqlSettings.Autentification.ToUpper() == "WINDOWS") ? this.SelectSqlData("tUser", false, "where Login = '" + this.SqlSettings.SqlUserConnect + "' AND Deleted = 0") : this.SelectSqlData("tUser", false, "where Login = SYSTEM_USER AND Deleted = 0")).Rows.Count <= 0)
                {
                    int num = (int)MessageBox.Show("Данный пользователь не зарегистрирован в базе " + this.SqlSettings.DBName +
                        "\nОбратитесь к админитсратору базы данных", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                }
...
#region Работа с ограничением потребителей
        ...
        private void тестЗагрузкиМоделиСхемыToolStripMenuItem_Click(object sender, EventArgs e)
        {
      ...
      , "Журнал нарушений");
        ...
        {
                this.ShowMdiForm(typeof(Abonent.FormJournalAbn));
        }
