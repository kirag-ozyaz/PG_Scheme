...
(select d.id idDoc, col.idAbnObj.value('@idAbnObj', 'int') idAbnObj, d.TypeDoc, d.NumRequest,  d.DateDoc
     ...
     , case 
		...
		where d.deleted = 0 and col.idAbnObj.value('@countPoint', 'varchar(max)') > 0 and d.isSendSiteMe =  1)
...
select doc.id id, doc.numRequestME num, Cast(netRegion.netRegionVAlue as int) name
							...
							where doc.id = d1.id  and d1.deleted = 0
									and (Case 
											...
											when dHV.PowerV in (6, 10) and (CHARINDEX('10 (10.5) кВ',c.value('@SubName','varchar(max)')) > 0 or CHARINDEX('6 (6.3) кВ',c.value('@SubName','varchar(max)')) > 0) then 1
												...
												--when dHv.PowerV in (0.4) and (CHARINDEX('10 (10.5)',c.value('@SubName','varchar(max)')) > 0 and CHARINDEX('6 (6.3)',c.value('@SubName','varchar(max)')) > 0) then 0
												--when dHv.PowerV in (0.4) and CHARINDEX('Ш04',c.value('@BusName','varchar(max)')) = 1 then 1
												--when dHv.PowerV in (6, 10) and (CHARINDEX('10 (10.5)',c.value('@SubName','varchar(max)')) > 0 or CHARINDEX('6 (6.3)',c.value('@SubName','varchar(max)')) > 0) then 1
												--when dHv.PowerV in (6, 10) and CHARINDEX('Ш04',c.value('@BusName','varchar(max)')) = 0 then 1
												--when dHV.iddamage is null then 1
	...
	, chCodeReasonOrg.name org_reason_code
	, chCodeReasonTech.name as tech_reason_code
	...
	, Stuff((select distinct '; ' +  chNameCol1.SocrName
		from tJ_DamageCharacter dh	
			left join tr_classifier chNameCol1 on chNameCol1.id = dh.col1
...
where doc.deleted = 0  and Year(doc.DateDoc) >= 2019 and doc.isSendSiteMe  = 1
		and doc.typeDoc <> 1874
