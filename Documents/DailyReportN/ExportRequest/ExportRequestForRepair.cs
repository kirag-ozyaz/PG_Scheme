...
using DataSql;
/// <summary>
/// отправка данных (ремонты) по почте
/// </summary>
internal static class ExportRequestForRepair
{
    internal static int IdODS= 920;

    internal static DataTable LoadData(SQLSettings sqlsettings, DateTime dateTripBeg, DateTime dateTripEnd, bool Crash)
	{
		string strDateBeg = "'" + dateTripBeg.Date.ToString("yyyyMMdd") + "'";
		string strDateEnd = "'" + dateTripEnd.Date.ToString("yyyyMMdd") + " 23:59:59'";
		...
		if (dateTripBeg.Date == dateTripEnd.Date)
		{
			foreach (DataRow dataRow in dataTable.Rows)
			{
				//DataRow dataRow = (DataRow)obj;
				if (dateTripBeg.Date > Convert.ToDateTime(dataRow["dateTripBeg"]))
				{
					dataRow["dateTRipBeg"] = dateTripBeg.Date;
				}
				if (dateTripEnd.Date.AddDays(1.0).AddMinutes(-1.0) < Convert.ToDateTime(dataRow["DateTripEnd"]))
				{
					dataRow["dateTripEnd"] = dateTripEnd.Date.AddDays(1.0).AddMinutes(-1.0);
				}
			}
		}
		return dataTable;
	...
	private static DataTable LoadData(SQLSettings sqlsettings, DateTime dateTripBeg, DateTime dateTripEnd)
	{
		string strDateBeg = "'" + dateTripBeg.Date.ToString("yyyyMMdd") + "'";
		string strDateEnd = "'" + dateTripEnd.Date.ToString("yyyyMMdd") + " 23:59:59'";
		string text3 = string.Concat(new string[]
    ...
    /// <summary>
    /// Создадим исходящий файл
    /// </summary>
    /// <returns></returns>
	private static bool WriteToFile(string FileName, StringBuilder stringBuilder, bool Crash = false, Encoding encoding = null)
	{
		if (encoding == null)
		{
		...
		try
		{
			...
			if (Crash)
			{
				streamWriter.Write(ExportRequestForRepair.ConvertHTMLToExcelXML(stringBuilder.ToString())); // AddCrashStringBuilder
            }
			else
			{
    ...
    /// старые планы работ
    /// </summary>
	public static bool ToHTML(SQLSettings sqlsettings, string FileName, DateTime DateBeg, DateTime DateEnd, bool Crash)
	{
		...
		stringBuilder.Append("<TD ALIGN=CENTER VALIGN=CENTER><FONT SIZE=2><B>Время</B></FONT>\n");
		stringBuilder.Append("</tr>\n");
		DataTable dataTable = ExportRequestForRepair.LoadData(sqlsettings, DateBeg, DateEnd, Crash);
		int idSR = -1;
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			DataRow dataRow = dataTable.Rows[i];
			string strDateBeg = dataRow["datetripbeg"].ToString();
			string strDateEnd = dataRow["datetripend"].ToString();
			int num2 = i;
			int num3 = 0;
			for (;;)
			{
			...
			{
				stringBuilder.Append("<TR>\n");
				string strDivision = ExportRequestForRepair.GetNameRaion(dataRow["SR"].ToString());
				stringBuilder.Append("<TD VALIGN=CENTER BGCOLOR=#c0dcc0 COLSPAN=7><FONT SIZE=2><I><B>" + strDivision + "</B></I></FONT>\n");
				stringBuilder.Append("</tr>\n");
			...
			string Address = (dataRow["Address"] == DBNull.Value || string.IsNullOrEmpty(dataRow["Address"].ToString())) ? "&nbsp;" : dataRow["Address"].ToString();
			stringBuilder.Append(string.Concat(new string[]
			{
				"<TD VALIGN=CENTER ROWSPAN=",
				(num3 + 1).ToString(),
    ...
    /// новые планы работ
    /// </summary>
	public static bool toPHP(SQLSettings sqlsettings, string FileName, DateTime DateBeg, DateTime DateEnd)
	{
		StringBuilder stringBuilder = new StringBuilder();
		...
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			...
			if (idSR != Convert.ToInt32(dataRow["idSR"]))
			{
				stringBuilder.Append("<TR>\n");
				string strDivision = ExportRequestForRepair.GetNameRaion(dataRow["SR"].ToString());
				stringBuilder.Append("<TD class=\"cell-sr\" COLSPAN=8 style=\"background-color:#c0dcc0;\">" + strDivision + "\n");
				stringBuilder.Append("</tr>\n");
				idSR = Convert.ToInt32(dataRow["idSR"]);
			}
			stringBuilder.Append("<TR>\n");
	...
	private static string GetNameRaion(string str)
	{
    ...
    /// план работ
    /// </summary>
	public static bool ToExcel(SQLSettings sqlsettings, string FileName, DateTime DateBeg, bool Crash)
	{
		DataTable table = ExportRequestForRepair.LoadData(sqlsettings, DateBeg, DateBeg, Crash);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.Append(ExportRequestForRepair.ExcelHeader());
		stringBuilder.Append(ExportRequestForRepair.ExcelStyles());
		stringBuilder.Append(ExportRequestForRepair.WriteFirstWorkSheet(table, DateBeg));
    ...
    /// <summary>
    /// создадим шапку книги
    /// </summary>
    /// <returns></returns>
	private static string ExcelHeader() // smethod_8
    {
		StringBuilder stringBuilder = new StringBuilder();
    ...
    /// <summary>
    /// зафиксируем стили ячеек на листах  в книге
    /// </summary>
    /// <returns></returns>
	private static string ExcelStyles() // smethod_9
    {
		StringBuilder stringBuilder = new StringBuilder();
    ...
    /// <summary>
    /// создадим листы  и наполним их данными
    /// </summary>
    /// <returns></returns>
	private static string WriteFirstWorkSheet(DataTable table, DateTime DateEvent) // 
    {
    ...
    /// </summary>
    internal static bool SendMessage(string host, int port, string userName, string password, string address, string displayName, List<Recipient> listMailAddress, string subject, string body, List<string> listAttachment = null, bool flagSSL = false)
	{
		try
		{
            ...
            foreach (Recipient stm in listMailAddress)
			{
				if (!string.IsNullOrEmpty(stm.Address))
				{
					mailMessage.To.Add(new MailAddress(stm.Address, stm.Name));
				}
			}
			...
			{
				foreach (string fileName in listAttachment)
				{
