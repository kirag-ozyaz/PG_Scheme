...
//        private int IdBus;
       ...
       // [CompilerGenerated]
        ...
        // запрос для поиска начальников сетевых районов (;GroupWorker;HeadRegion;)
        /*
        SELECT worker.[FIO]
              ...
              , HeadDivision.[FIO]
          FROM [GES].[dbo].[vP_Worker] as worker
          left join[GES].[dbo].[tR_Division] as division on division.code = worker.[CodeDivision]
        
          left join
        
            (select[FIO],[CodeDivision]
                from [GES].[dbo].[vP_Worker]
        WHERE ParentKey = ';GroupWorker;HeadRegion;' AND DateEnd IS NULL) as HeadDivision on HeadDivision.[CodeDivision] = worker.[CodeDivision]
  WHERE ParentKey = ';GroupWorker;Meas;' AND DateEnd IS NULL
