...
namespace Documents.Measurement
{
    internal partial class FormMeasurementReports : FormBase
    {
        ...
        // ReportType.CoefficientLoading        = Анализ коэффициента загрузки
        // ReportType.CoefficientIrregularity   = Анализ коэффициента неравномерности фаз
        ...
        {
            this.dsMeasurementN.EnforceConstraints = false;
            this.CreateReport();
        }
        ...
        private string GetReportURI(ReportType type)
        {
            switch (type)
            {
                case ReportType.MeasurementLowVoltage:
                    ...
                    {
                        return "Measurement.Documents.Report.ReportMeasLowVoltage.rdlc";
                    }
                    return "Measurement.Documents.Report.ReportMeasLowVoltageWithOutNameObj.rdlc";

                case ReportType.MeasurementLowVoltageSP:
                    return "Measurement.Documents.Report.ReportMeasLowVoltageSP.rdlc";

                case ReportType.MeasurementHighVoltage:
                    return "Measurement.Documents.Report.ReportMeasHighVoltage.rdlc";

                case ReportType.ExcelessLoad:
                    return "Measurement.Documents.Report.ReportExcelessLoad.rdlc";

                case ReportType.CoefficientLoading:
                    return "Measurement.Documents.Report.ReportCoefficientLoading.rdlc";

                case ReportType.CoefficientIrregularity:
                    return "Measurement.Documents.Report.ReportCoefficientIrregularity.rdlc";

                case ReportType.TransformerBySubstation:
                    // return "Documents.Forms.Measurement.";
                    return "Measurement.Documents.Report.";
            }
            return "";
        }
        ...
        {
            /// Так было
            /// не рассчитывались коэффициенты и не загружалась мощность трансформаторов
            ...
            string str = (this.IdNetworkAreas == -1) ? "" : (" AND idNetRegion = " + IdNetworkAreas.ToString());
            string str2 = (this.PowerTransf == -1) ? "" : (" AND Power " + (this.IsPowerLarge ? "> " : "< ") + this.PowerTransf.ToString());
            string str5 = string.Format(" WHERE Year = {0}{1}{2} ORDER BY NetRegion, SSSocrName, Substation , Transformer", new object[]
            {
            this.Year,
            str,
            str2
            ...
            //SelectSqlData(this.dsMeasurementN, dsMeasurementN.vJ_CoefficientLoading, true, str5);
            // Соберем замеры за период
            var dt_vJ_CoefficientLoading = SelectSqlData("vJ_CoefficientLoading", true, str5);
            string stridMeasurement = string.Join(",", (from r in dt_vJ_CoefficientLoading.AsEnumerable() select r.Field<int>("idMeasurement").ToString()).Distinct().ToArray<string>());
            // Соберем данные по трансформаторам (Inom, Power)
            string stridTransf = string.Format("Where id IN ({0})", string.Join(",", (from r in dt_vJ_CoefficientLoading.AsEnumerable()
                                                   select r.Field<int>("idTransf").ToString())
                                                   .Distinct()
                                                   .ToArray<string>()
                                                   ));

            
            var dt_fn_J_MeasTransfPassport = SelectSqlData("vJ_MeasTransfPassportN", true, stridTransf);

            // Соберем данные для замеров
            string str6 = string.Format("WHERE idMeasurement IN({0})", stridMeasurement);
            //var dt_vJ_MeasCableReport2 = SelectSqlData("vJ_MeasCableReport2", true, str6);
            this.dsMeasurementN.vJ_MeasCableReport2.Clear();
            SelectSqlData(this.dsMeasurementN, this.dsMeasurementN.vJ_MeasCableReport2, true, str6);

            var res1 = dsMeasurementN.vJ_MeasCableReport2
                .Select(row => new
                {
                    idMeasurement = (row["idMeasurement"] != DBNull.Value ? row["idMeasurement"] : -1),
                    idBus = ((row["idBus"] != DBNull.Value) ? row["idBus"] : -1),
                    idTransf = (row["idTransf"] != DBNull.Value ? row["idTransf"] : -1),
                    Iad = Convert.ToInt32(row["Iad"] != DBNull.Value ? row["Iad"] : 0),
                    Ian = Convert.ToInt32(row["Ian"] != DBNull.Value ? row["Ian"] : 0),
                    Ibd = Convert.ToInt32(row["Ibd"] != DBNull.Value ? row["Ibd"] : 0),
                    Ibn = Convert.ToInt32(row["Ibn"] != DBNull.Value ? row["Ibn"] : 0),
                    Icd = Convert.ToInt32(row["Icd"] != DBNull.Value ? row["Icd"] : 0),
                    Icn = Convert.ToInt32(row["Icn"] != DBNull.Value ? row["Icn"] : 0),
                    Iod = Convert.ToInt32(row["Iod"] != DBNull.Value ? row["Iod"] : 0),
                    Ion = Convert.ToInt32(row["Ion"] != DBNull.Value ? row["Ion"] : 0)
                }).Distinct();

            var res3 = res1.GroupBy(row => new { row.idMeasurement, row.idBus, row.idTransf })
               .Select(q => new
               {
                   idMeasurement = q.Key.idMeasurement,
                   idBus = q.Key.idBus,
                   idTransf = q.Key.idTransf,
                   Iad = q.Sum(row => row.Iad),
                   Ian = q.Sum(row => row.Ian),
                   Ibd = q.Sum(row => row.Ibd),
                   Ibn = q.Sum(row => row.Ibn),
                   Icd = q.Sum(row => row.Icd),
                   Icn = q.Sum(row => row.Icn),
                   Iod = q.Sum(row => row.Iod),
                   Ion = q.Sum(row => row.Ion),

                   // Inom = dt_fn_J_MeasTransfPassport.AsEnumerable().Where(r => Convert.ToInt32(r["ID"]) == Convert.ToInt32(q.Key.idTransf)).Select(s=> s["INOM"]).FirstOrDefault(),
                   param = dt_fn_J_MeasTransfPassport.AsEnumerable().Where(r => Convert.ToInt32(r["ID"]) == Convert.ToInt32(q.Key.idTransf)).Select(s => new { Inom = s["Inom"] != DBNull.Value ? s["Inom"] : 0, Power = s["Power"] != DBNull.Value ? s["Power"] : 0 }).FirstOrDefault()
               });

            var res4 = res3.Select(qqc1 => new
            {
                idMeasurement = qqc1.idMeasurement,
                idBus = qqc1.idBus,
                idTransf = qqc1.idTransf,
                Iad = Convert.ToDecimal(qqc1.Iad),
                Ian = Convert.ToDecimal(qqc1.Ian),
                Ibd = Convert.ToDecimal(qqc1.Ibd),
                Ibn = Convert.ToDecimal(qqc1.Ibn),
                Icd = Convert.ToDecimal(qqc1.Icd),
                Icn = Convert.ToDecimal(qqc1.Icn),
                Inom = qqc1.param != null ? Convert.ToDecimal(qqc1.param.Inom) : 0,
                Power = qqc1.param != null ? qqc1.param.Power : 0,
                //Param2 = dt_fn_J_MeasTransfPassport.AsEnumerable().Where(r => Convert.ToInt32(r["ID"]) == Convert.ToInt32(qqc1.idTransf)).Select(s => new { Inom = s["Inom"], Power = s["Power"] }).FirstOrDefault(),
                Icpd = Convert.ToDecimal(Math.Round(Convert.ToDecimal(qqc1.Iad + qqc1.Ibd + qqc1.Icd) / 3, MidpointRounding.AwayFromZero)),
                Icpn = Convert.ToDecimal(Math.Round(Convert.ToDecimal(qqc1.Ian + qqc1.Ibn + qqc1.Icn) / 3, MidpointRounding.AwayFromZero)),
                num16 = Math.Max(Math.Max(Math.Abs(qqc1.Iad - qqc1.Ibd), Math.Abs(qqc1.Iad - qqc1.Icd)), Math.Abs(qqc1.Ibd - qqc1.Icd)),
                num17 = Math.Max(Math.Max(Math.Abs(qqc1.Ian - qqc1.Ibn), Math.Abs(qqc1.Ian - qqc1.Icn)), Math.Abs(qqc1.Ibn - qqc1.Icn))
            })
                .Select(q => new
                {
                    idMeasurement = q.idMeasurement,
                    idBus = q.idBus,
                    idTransf = q.idTransf,
                    Power = q.Power,
                    Kzd = (q.Inom != 0) ? Math.Round(q.Icpd / (q.Inom / 100), MidpointRounding.AwayFromZero) : 0,
                    Kzn = (q.Inom != 0) ? Math.Round(q.Icpn /(q.Inom / 100), MidpointRounding.AwayFromZero) : 0,
                    Kfd = (Convert.ToDecimal(q.Iad + q.Ibd + q.Icd) != 0) ? Math.Round((q.num16 / (Convert.ToDecimal(q.Iad + q.Ibd + q.Icd) / 100)), MidpointRounding.AwayFromZero) : 0,
                    Kfn = (Convert.ToDecimal(q.Ian + q.Ibn + q.Icn) != 0) ? Math.Round((q.num17 / (Convert.ToDecimal(q.Ian + q.Ibn + q.Icn) / 100)), MidpointRounding.AwayFromZero) : 0
                });


            var res5 = dt_vJ_CoefficientLoading.AsEnumerable()
               .GroupJoin(
                  res4.AsEnumerable(),
                  ...
                  (c, grp) => new
                  {
                      NetRegion = c["NetRegion"],
                      IdNetRegion = c["IdNetRegion"],
                      Year = c["Year"],
                      SSSocrName = c["SSSocrName"],
                      Substation = c["Substation"],
                      Transformer = c["Transformer"],
                      // Power = dt_fn_J_MeasTransfPassport.AsEnumerable().Where(r => (int)r["Id"] == (int)c["IdTransf"]).First()["Power"],
                      Power = grp.Count() == 0 ? 0 : grp.FirstOrDefault().Power,
                      Kntd = grp.Count() == 0 ? 0 : grp.FirstOrDefault().Kzd,
                      Kntn = grp.Count() == 0 ? 0 : grp.FirstOrDefault().Kzn,
                      Kpfd = grp.Count() == 0 ? 0 : grp.FirstOrDefault().Kfd,
                      Kpfn = grp.Count() == 0 ? 0 : grp.FirstOrDefault().Kfn,
                  }
               )
            ...
            // Выборка
            // Power
            var res6 = (this.PowerTransf != -1) ? (this.IsPowerLarge ? res5.Where(r => this.PowerTransf < Convert.ToInt32(r.Power)) : res5.Where(r => this.PowerTransf > Convert.ToInt32(r.Power))) : res5;
            // Kntd
            var res7 = this.IsDay ? (this.IsCoefficientLarge ? res6.Where(r => this.CoefficientLoading < Convert.ToInt32(r.Kntd)) : res6.Where(r => this.CoefficientLoading > Convert.ToInt32(r.Kntd))) : res6;
            // Kntn
            var res8 = this.IsNight ? (this.IsCoefficientLarge ? res7.Where(r => this.CoefficientLoading < Convert.ToInt32(r.Kntn)) : res7.Where(r => this.CoefficientLoading > Convert.ToInt32(r.Kntn))) : res7; 
           
                //return new BindingSource { DataSource = this.dsMeasurementN.vJ_CoefficientLoading };
                return new BindingSource { DataSource = res8 };
        }
        ...
        private BindingSource GetCoefficientIrregularityData()
        {
            ...
            //SelectSqlData(this.dsMeasurementN, dsMeasurementN.vJ_CoefficientLoading, true, str5);
            // Соберем замеры за период
            var dt_vJ_CoefficientIrregularity = SelectSqlData("vJ_CoefficientIrregularity", true, str5);
            string stridMeasurement = string.Join(",", (from r in dt_vJ_CoefficientIrregularity.AsEnumerable() select r.Field<int>("idMeasurement").ToString()).Distinct().ToArray<string>());
            // Соберем данные по трансформаторам (Inom, Power)
            string stridTransf = string.Format("Where id IN ({0})", string.Join(",", (from r in dt_vJ_CoefficientIrregularity.AsEnumerable()
                                                                                      select r.Field<int>("idTransf").ToString())
                                                   .Distinct()
                                                   .ToArray<string>()
                                                   ));
            ...
            var res5 = dt_vJ_CoefficientIrregularity.AsEnumerable()
               .GroupJoin(
                  res4.AsEnumerable(),
                  c => new { IdMeasurement = c["IdMeasurement"], IdBus = c["IdBus"], IdTransf = c["IdTransf"] },
                  v => new { IdMeasurement = v.idMeasurement, IdBus = v.idBus, IdTransf = v.idTransf },
            ...
            var res6 = (this.PowerTransf != -1) ? (this.IsPowerLarge ? res5.Where(r => this.PowerTransf < Convert.ToInt32(r.Power)) : res5.Where(r => this.PowerTransf > Convert.ToInt32(r.Power))) : res5;
            // Kntd
            var res7 = this.IsDay ? (this.IsCoefficientLarge ? res6.Where(r => this.CoefficientLoading < Convert.ToInt32(r.Kpfd)) : res6.Where(r => this.CoefficientLoading > Convert.ToInt32(r.Kpfd))) : res6;
            // Kntn
            var res8 = this.IsNight ? (this.IsCoefficientLarge ? res7.Where(r => this.CoefficientLoading < Convert.ToInt32(r.Kpfn)) : res7.Where(r => this.CoefficientLoading > Convert.ToInt32(r.Kpfn))) : res7;
