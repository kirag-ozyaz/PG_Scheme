...
{
    public static class ExportRequestForRepair
    {
        ...
        /// <param name="listMailAddress"></param>
        /// <param name="subject"></param>
        /// <param name="body"></param>
        /// <param name="listAttachment"></param>
        /// <param name="flagSSL"></param>
        /// <returns></returns>
        public static bool SendMessage(string host, int port, string userName, string password, string address, string displayName, List<RequestForRepair.StructMailAddress> listMailAddress, string subject, string body, List<string> listAttachment = null, bool flagSSL = false)
        {
            try
            {
                ...
                foreach (RequestForRepair.StructMailAddress stm in listMailAddress)
                {
                    if (!string.IsNullOrEmpty(stm.address))
                    {
                        message.To.Add(new MailAddress(stm.address, stm.displayName));
                    }
                }
                ...
                {
                    using (List<string>.Enumerator enumerator = listAttachment.GetEnumerator())
                    {
                        while (enumerator.MoveNext())
                        {
                            Attachment item = new Attachment(enumerator.Current, "application/octet-stream");
