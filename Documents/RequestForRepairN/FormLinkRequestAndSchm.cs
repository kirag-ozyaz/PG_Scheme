...
internal partial class FormLinkRequestAndSchm : FormBase
{
    ...
    private void FormLinkRequestAndSchm_Load(object sender, EventArgs e)
	{
		...
		{
			this.listBoxObjects.Items.Add(new FormLinkRequestAndSchm.SchmObject(num, this.LinkObjects[num]));
		}
	}
    /// <summary>
	...
	{
        RequestForRepairN.DataSets.DataSetDamage ds = new RequestForRepairN.DataSets.DataSetDamage();
    ...
    private void treeViewSubstation_BeforeExpand(object sender, TreeViewCancelEventArgs e)
    {
        ...
        {
            e.Node.Nodes.Clear();
            RequestForRepairN.DataSets.DataSetDamage ds;
            switch (e.Node.Level)
            {
                case 1:
                    {
	...
	{
        RequestForRepairN.DataSets.DataSetDamage ds = new RequestForRepairN.DataSets.DataSetDamage();
		base.SelectSqlData(ds, ds.tSchm_Relation, true, " where Edge = " + node.Tag.ToString());
		if (ds.tSchm_Relation.Rows.Count > 0)
		{
			string text = "";
	...
	{
        RequestForRepairN.DataSets.DataSetDamage ds = new RequestForRepairN.DataSets.DataSetDamage();
		base.SelectSqlData(ds, ds.vSchm_TreeCellLine, true, " where ParentLink = " + node.Tag.ToString());
		foreach (DataRow dataRow in ds.vSchm_TreeCellLine)
		{
	...
	{
		foreach (SchmObject obj in this.listBoxObjects.SelectedItems)
		{
            this.LinkObjects.Remove(obj.Id);
        }
		this.listBoxObjects.Items.Clear();
		foreach (int num in this.LinkObjects.Keys)
		{
	...
	private void tooBtnFind_Click(object sender, EventArgs e)
	{
		...
		TreeNode treeNode = this.SearchNode(list_, this.treeViewSubstation.SelectedNode, this.toolTxtFind.Text, (FormLinkRequestAndSchm.eFind.First));
		if (treeNode != null)
		{
	...
	private void toolBtnFindNext_Click(object sender, EventArgs e)
	{
		List<TreeNode> list_ = new List<TreeNode>();
		list_ = this.AddNodeInList(this.treeViewSubstation.Nodes, list_);
		TreeNode treeNode = this.SearchNode(list_, this.treeViewSubstation.SelectedNode, this.toolTxtFind.Text, (FormLinkRequestAndSchm.eFind.Next));
		if (treeNode != null)
		{
			this.treeViewSubstation.SelectedNode = treeNode;
			this.treeViewSubstation.SelectedNode.Expand();
	...
	private void toolBtnFindPrev_Click(object sender, EventArgs e)
	{
		List<TreeNode> list_ = new List<TreeNode>();
		list_ = this.AddNodeInList(this.treeViewSubstation.Nodes, list_);
		TreeNode treeNode = this.SearchNode(list_, this.treeViewSubstation.SelectedNode, this.toolTxtFind.Text, (FormLinkRequestAndSchm.eFind.Prev));
		if (treeNode != null)
		{
			this.treeViewSubstation.SelectedNode = treeNode;
			this.treeViewSubstation.SelectedNode.Expand();
	...
	private TreeNode SearchNode(List<TreeNode> nodelist, TreeNode selNode, string text, FormLinkRequestAndSchm.eFind e)
	{
