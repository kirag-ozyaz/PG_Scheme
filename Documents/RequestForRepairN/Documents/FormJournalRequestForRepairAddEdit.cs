...
/// <summary>
/// Документ аварийные заявки и на ремонт оборудования (N3)
/// </summary>
public partial class FormJournalRequestForRepairAddEdit : FormBase
{
    ...
    /// <summary>
    /// Статус заявки перед передачей на сайт, РИЦ или в ОДС
    /// </summary>
    ...
    public FormJournalRequestForRepairAddEdit(int idDocument, int idDivision, eActionRequestRepair action, bool isCrash = false)
    {
        this.InitializeComponent();
        this.CmdDataBindingsAdd();
    ...
    /// <summary>
    /// Статус на соединение  с FTP сервером РИЦ
    /// </summary>
    private void ConnectGetStatusSendRICToFTPRIC() // GetStatusSendRIC
    {
...
#pragma warning restore CS0618 // Тип или член устарел
    ...
    {
        #region Блокировка формы от редактирования (новые не создаем)
        if (this.eAction != eActionRequestRepair.Add)
        {
            if (this.IdDocument > 0)
            {
                // получим ид документа (записанного)
                DataTable table = new DataTable();
                table.TableName = "tJ_RequestForRepair";
                base.SelectSqlData(table, true, " where id = " + this.IdDocument.ToString());
                // Найдем роли текущего юзера
                Library.Security security = new Library.Security();
                security.SqlSettings = SqlSettings;
                var CurrentRules = security.GetCurrentUserRules();
                var res = CurrentRules.AsEnumerable().Where(w => w["ParentKey"].ToString() == $";GroupWorker;ReconcileRequestRepair;");
                // проверим имеет ли право текущий пользователь права соглсования данного документа
                // 1. Надо проверить, если документ уже создан и Status не пустой, то документ должен быть закрыт 
                //    от редактирования для "создателей документа"
                //    для "отправителей" и "корректоров" документ должен быть достпен, до тех пор пока не отправится на сайт
                if (res.Count() == 0)
                {
        ...
        //**********************
#if !DEBUG
        chkSendSiteME.Visible = false;
#else
        chkSendSiteME.Visible = true;
#endif
        //*****************
        if (this.dataSetDamage.tJ_RequestForRepair.Rows.Count > 0)
        {
            isSendSiteMe = this.dataSetDamage.tJ_RequestForRepair.Rows[0]["isSendSiteMe"];
            ...
            else if ((int)isSendSiteMe == 3 || (int)isSendSiteMe == 6)
            {
    ...
    private void chkSendSiteME_CheckedChanged(object sender, EventArgs e)
    {
        ...
        else
        {
            if ((int)isSendSiteMe == 1)
                isSendSiteMe = this.chkSendSiteME.Checked ? 1 : 2;
            else if ((int)isSendSiteMe == 3 || (int)isSendSiteMe == 6 || (int)isSendSiteMe == 5 || (int)isSendSiteMe == 4)
            { }
            else
        ...
        this.dataSetDamage.tJ_RequestForRepair.Rows[0]["isSendSiteMe"] = isSendSiteMe;
    }
    private void chkSendToSite_CheckedChanged(object sender, EventArgs e)
    {
       if (((CheckBox)sender).CheckState == CheckState.Checked)
       {
            chkSendSiteME.Enabled = true;
    ...
    private bool CheckRequiredFields()
    {
        ...
        if (this.checkBoxAgreed.Checked && this.IdDivision == ODS_DIVISION)
        {
    ...
    private void backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
    {
        ...
        foreach (var obj in eLinkObjects)
        {
            ...
            DataTable table = FormJournalRequestForRepairAddEdit.FillTableTransOff(listTrans, new StaticFormData { Sqlsettings = this.SqlSettings, eForm = this });
            ...
            //ReferenceTables.CreateReferenceTables.LoadTableSсhmAbnFullKladr(this.tableSchmAbnFull, this.SqlSettings, listElectricObj, dateTime, false);
            Reference.Tables.LoadTableSChmAbnFull_2019(this.tableSchmAbnFull, this.SqlSettings, listElectricObj, dateTime, false);
        ...
        if (this.tableKladrObjAddress == null)
        {
            ...
            this.tableKladrObjAddress.Columns.Add("idCity", typeof(int));
    ...
    private void backgroundWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
    {
        ...
        try
        {
            var enumAddress = tableSchmAbnFull.AsEnumerable().GroupBy(r => new { idCity = r["idCity"], idStreet = r["idStreet"], house = r["house"] });
            foreach (var row in enumAddress)
                {
                    ...
                    })).Length == 0)
                    {
                        ...
                        dataRow2["idRequest"] = this.IdDocument;
                        dataRow2["idKladrObj"] = row.Key.idCity;
    ...
    private void isCountRowsForRepairDaily()
    {
        ...
        if (new SqlDataCommand(this.SqlSettings).SelectSqlData(string.Format("" +
            ...
            "   as (select id, isApply,  0 as level  from tj_damage   where idReqForRepair = {0} " +
            "       union all " +
            "       select tj_damage.id, tJ_Damage.isApply, level + 1 from tj_damage " +
            "            inner join tree on tree.id = tj_damage.idParent)  " +
            "              select * from tree   " +
    ...
    private void FillSubTablesDocumentDamageForCommentXml(DataRow dataRowCureent, DateTime dateStatusOperational, int idSchmObj)
    {
        ...
        List<int> listTrans = new List<int>();
        FormJournalRequestForRepairAddEdit.FillTreeNodeTrans(treeNodeObj_, listTrans);
        //if (idTypeDamage == (int)eTypeDocuments.DamageHV)
        //{
            DataTable dataTable_ = FormJournalRequestForRepairAddEdit.FillTableTransOff(listTrans, new StaticFormData { Sqlsettings = this.SqlSettings, eForm = this });
    ...
    private void FillSubTablesDocumentDamage2(int idDamage, DateTime dateStatusOperational, int idSchmObj)
    {
        ...
        if (idTypeDamage == (int)eTypeDocuments.DamageHV)
        {
            ...
            List<int> listTrans = new List<int>();
            FormJournalRequestForRepairAddEdit.FillTreeNodeTrans(treeNodeObj_, listTrans);
            DataTable dataTable = FormJournalRequestForRepairAddEdit.FillTableTransOff(listTrans, new StaticFormData { Sqlsettings = this.SqlSettings, eForm = this });
            // коэффициенты
    ...
    private static void FillTreeNodeTrans(TreeNodeObj treeNodeObj, List<int> listTrans)
    {
        ...
        {
            FormJournalRequestForRepairAddEdit.FillTreeNodeTrans(Obj, listTrans);
        }
    }
    /// <summary>
    ...
    private static DataTable FillTableTransOff(List<int> listTrans, StaticFormData Me)
    {
        ...
        {
            sqlConnection.Open();
            using (SqlCommand sqlCommand = new SqlCommand(ResourceReader.GetStringAssembly(Me.eForm.GetType(), "RequestForRepairN.GetTransformerSchmObj.sql"), sqlConnection))
            {
...
#region блок работы с отключенными подстанциями
    ...
    private void FillTableAbnOff(TreeDataGridViewNode SubNodeTree = null)
    {
        ...
        var result = from r in tableSchmAbnFull.AsEnumerable()
                     orderby r["idCity"], r["idStreet"], r["house"].ToString()
                     select r;
        ...
        foreach (DataRow row in result)
        {
            ...
            DataRow[] arrayObj = this.tableKladrObj.Select("id = " + row["idCity"].ToString());
            ...
            if (idSity != (int)row["idCity"])
            {
                idSity = (int)row["idCity"];
                SubNodeTreeSity = this.AddTreeParametrAbn(this.dgvAbn.Nodes, new object[] { City });
                SubNodeTreeSity.Cells[this.idCityDataGridViewTextBoxColumn.Index].Value = row["idCity"];
            }
            ...
            if (idStreet != (int)row["idStreet"])
            {
                ...
                SubNodeTreeStreet.Cells[this.idCityDataGridViewTextBoxColumn.Index].Value = row["idCity"];
            ...
            if (idHouse != (string)row["house"])
            {
                ...
                SubNodeTreeHouse.Cells[this.idCityDataGridViewTextBoxColumn.Index].Value = row["idCity"];
            ...
            SubNodeTreeAbnObj.Cells[this.voltageDgvColumn.Index].Value = row["voltage"];
            ...
            SubNodeTreeAbnObj.Cells[this.idCityDataGridViewTextBoxColumn.Index].Value = row["idCity"];
            ...
            SubNodeTreeAbnObj.Cells[this.idAbnNetDataGridViewTextBoxColumn.Index].Value = row["idAbnNet"];
            SubNodeTreeAbnObj.Cells[this.nameAbnNetDataGridViewTextBoxColumn.Index].Value = row["nameAbnNet"];
    ...
    /// <summary>
    /// Сохранение xml-данных трнасформатора и подстанций
    /// </summary>
            ...
            if (!string.IsNullOrEmpty(dataGridViewColumn.DataPropertyName))
            {
                ...
                if (xmlAttribute != null && !string.IsNullOrEmpty(xmlAttribute.Value))
                {
                    ...
                    if (dataGridViewColumn.DataPropertyName == "typeAbn")
                        treeDataGridViewNode.Cells[dataGridViewColumn.Index].Value = Convert.ToInt32(Value);
                    else if (dataGridViewColumn.DataPropertyName == "PowerSet")
                    {
                        if (Decimal.TryParse(Value, out decimal PowerSetD) == false)
                        {
                            if (Value.IndexOf(".") > 0)
                                PowerSetD = Convert.ToDecimal(xmlAttribute.Value.Replace(".", System.Globalization.CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator));
                            else if (Value.IndexOf(",") > 0)
                                PowerSetD = Convert.ToDecimal(xmlAttribute.Value.Replace(",", System.Globalization.CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator));
                            else
                                PowerSetD = 0;
                        }
                        treeDataGridViewNode.Cells[dataGridViewColumn.Index].Value = PowerSetD;
                        dataRow[dataGridViewColumn.DataPropertyName] = PowerSetD;
                    }
                    else
                        treeDataGridViewNode.Cells[dataGridViewColumn.Index].Value = Value;

                    if ((dataGridViewColumn.DataPropertyName == "idAbn" && xmlAttribute.Value != null)  || ((dataGridViewColumn.DataPropertyName != "idAddress") && (dataGridViewColumn.DataPropertyName != "PowerSet")))
                        dataRow[dataGridViewColumn.DataPropertyName] = Value;
    ...
    {
        if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && eAction != eActionRequestRepair.Read && chkSendSiteME.CheckState == CheckState.Checked /*&& !this.chkSendToSite.Checked*/)
        {
            if (((ControlsLbr.TreeDataGridView)sender).CurrentRow.Cells[0].Value.ToString() != ">")
                return;
            UpDateTableScmAbnFull(e.ColumnIndex, e.RowIndex, dgvAbn[e.ColumnIndex, e.RowIndex]);
        }
    }
    private void dgvAbn_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
    {
        if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && eAction != eActionRequestRepair.Read && chkSendSiteME.CheckState == CheckState.Checked/*&& !this.chkSendToSite.Checked*/)
        {
            if (((ControlsLbr.TreeDataGridView)sender).CurrentRow.Cells[0].Value.ToString() != ">") { e.Cancel = true; return; }
            if (!(e.ColumnIndex == this.countPointDgvColumn.Index && e.RowIndex >= 0)) { e.Cancel = true; return; }
        }
        else { e.Cancel = true; return; }
    }
    private void dgvAbn_MouseDoubleClick(object sender, MouseEventArgs e)
    {
    }
    ...
    {
        if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && e.Button == MouseButtons.Left && eAction != eActionRequestRepair.Read && chkSendSiteME.CheckState == CheckState.Checked /* && !this.chkSendToSite.Checked*/)
        {
            if (((ControlsLbr.TreeDataGridView)sender).CurrentRow.Cells[0].Value.ToString() != ">") return;
            if (e.ColumnIndex == this.countPointDgvColumn.Index && e.RowIndex >= 0)
            {
                // получаем координаты и размеры ячейки
                var rect = dgvAbn.GetCellDisplayRectangle(e.ColumnIndex, e.RowIndex, true);
                // Внимание! Элемент не удаляется! Будет утечка памяти!
                this.KeyPreview = true;
                var changedCell = dgvAbn[e.ColumnIndex, e.RowIndex];
                var textBox = new TextBox
                {
                    Parent = dgvAbn,
                    Location = rect.Location,
                    Size = rect.Size,
                    Text = changedCell.Value.ToString()
                };
                textBox.PreviewKeyDown += (ss, ee) =>
                {
                    if (ee.KeyCode == Keys.Enter)
                    {
                        changedCell.Value = textBox.Text;
                        UpDateTableScmAbnFull(e.ColumnIndex, e.RowIndex, changedCell);
                        for (int index = this.dgvAbn.Controls.Count - 1; index >= 0; index--)
                            if (this.dgvAbn.Controls[index] is TextBox)
                                this.dgvAbn.Controls.RemoveAt(index);
    ...
    private void UpDateTableScmAbnFull(int ColumnIndex, int RowIndex, DataGridViewCell changedCell)
    {
        string strWhere = "";
        if (dgvAbn[this.idCityDataGridViewTextBoxColumn.Index, RowIndex].Value != null) strWhere += "idCity = " + dgvAbn[this.idCityDataGridViewTextBoxColumn.Index, RowIndex].Value;
        if (dgvAbn[this.idStreetDataGridViewTextBoxColumn.Index, RowIndex].Value != null) strWhere += " and idStreet = " + dgvAbn[this.idStreetDataGridViewTextBoxColumn.Index, RowIndex].Value;
        if (dgvAbn[this.housesDataGridViewTextBoxColumn.Index, RowIndex].Value != null) strWhere += " and house = '" + dgvAbn[this.housesDataGridViewTextBoxColumn.Index, RowIndex].Value + "'";
        if (dgvAbn[this.idAbnObjDataGridViewTextBoxColumn.Index, RowIndex].Value != null) strWhere += " and idAbnObj = " + dgvAbn[this.idAbnObjDataGridViewTextBoxColumn.Index, RowIndex].Value;
        if (dgvAbn[this.idSchmObjDataGridViewTextBoxColumn.Index, RowIndex].Value != null) strWhere += " and idSchmObj = " + dgvAbn[this.idSchmObjDataGridViewTextBoxColumn.Index, RowIndex].Value;

        var foundRows = tableSchmAbnFull.Select(strWhere);
        if (foundRows.Count() > 0)
        {
            foundRows[0]["CountPoint"] = changedCell.Value.ToString();
    ...
    private void toolBrnDelAbn_Click(object sender, EventArgs e)
    {
        if (this.dgvAbn.SelectedRows.Count > 0 && MessageBox.Show("Вы действительно хотите удалить выбранные строки?", "Удаление", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        {
            ...
            {
                string strWhere = "";
                if (node.Cells[this.idCityDataGridViewTextBoxColumn.Index].Value != null) strWhere += "idCity = " + node.Cells[this.idCityDataGridViewTextBoxColumn.Index].Value;
                ...
                if (node.Cells[this.idSchmObjDataGridViewTextBoxColumn.Index].Value != null) strWhere += " and idSchmObj = " + node.Cells[this.idSchmObjDataGridViewTextBoxColumn.Index].Value;
